  ...
  for (const ledger of ledgers) {
    ...
    if (actual !== expected.debtAmount) {
      mismatches.push({
        ...
        ledgerAmount: actual,
        expectedDebt: expected.debtAmount,
        ...
        diff: actual - expected.debtAmount,
        reason: expected.rewardAmount > 0 && actual === normalizeDebtAmount(expected.receivableAmount - expected.cashAmount - expected.bankAmount - expected.returnAmount) ? 'REWARD_NOT_DEDUCTED' : 'DELIVERY_DEBT_MISMATCH'
      });
    }
  }
  return {
    title: 'DELIVERY_CLOSEOUT_REWARD_DEBT_MISMATCH_AUDIT',
    dryRun: true,
