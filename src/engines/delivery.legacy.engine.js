...
amount:0})}function B(e={},r=[]){for(const t of r){const r=h(e[t]);if(r&&!["all","tat ca","tất cả","*"].includes(N(r)))return r}return""}function L(e={},r=[]){return r.flatMap(r=>{
const t=e[r];return Array.isArray(t)?t:[t]}).map(h).filter(Boolean)}function q(e={},r="",t=[]){const n=A(r),a=N(r);return!n&&!a||L(e,t).some(e=>{const r=A(e),t=N(e)
...
;function P(e=[],r={}){
const t=B(r,["deliveryStaffCode","deliveryStaffName","deliveryStaff","deliveryStaffKeyword","deliveryCode","deliveryName","nvgh","nvghCode","nvghName"]),n=B(r,["salesStaffCode","salesStaffName","salesStaff","salesStaffKeyword","salesCode","salesName","nvbh","nvbhCode","nvbhName"])
...
const t=B(e,"delivery"===r?["deliveryStaffCode","deliveryCode","nvghCode","staffDeliveryCode"]:["salesStaffCode","salesmanCode","salesCode","nvbhCode"]),n=B(e,"delivery"===r?["deliveryStaffName","deliveryStaff","deliveryStaffKeyword","deliveryName","nvgh","nvghName"]:["salesStaffName","salesStaff","salesStaffKeyword","salesName","nvbh","nvbhName"]),a=t||(U(n)?n:"")
...
;function Y(e={}){return B(e,["deliveryStaffCode","deliveryCode","nvghCode","staffDeliveryCode"])}function Z(e={}){
...
return h(e.actorDeliveryStaffCode||e.actorStaffCode||e.authenticatedStaffCode||"")}function ye(e={}){return Boolean(e&&e.enforceDeliveryOwnership)}function Ce(e={}){
return h(e.deliveryStaffCode||e.deliveryCode||e.nvghCode||e.shipperCode||e.driverCode||e.staffDeliveryCode)}function ve(e={},r={}){if(!ye(r))return!0;const t=me(r),n=Ce(e)
;return Boolean(t&&n&&A(n)===A(t))}function he(e=[],r={}){return ye(r)?(Array.isArray(e)?e:[]).filter(e=>ve(e,r)):e}function Se(e={},r={}){if(!ye(r))return;const t=me(r),n=Ce(e)
...
const e=new Error("Đơn giao hàng không thuộc nhân viên đang đăng nhập");throw e.status=403,e.code="DELIVERY_ORDER_FORBIDDEN",e}}function Oe(e){const r=h(e);if(!r)return null
;const t=[{id:r},{code:r},{orderCode:r},{salesOrderId:r},{salesOrderCode:r}];return/^[a-f\d]{24}$/i.test(r)&&t.push({_id:r}),{$or:t}}function ge(e,r={}){
const t=Oe(e),n=void 0!==r.version&&null!==r.version&&""!==r.version,a=n?Number(r.version):0;return{$and:[t,n?{version:a}:{$or:[{version:{$exists:!1}},{version:0},{version:null}]}]
}}function pe(e){if(e)return e;const r=new Error("Dữ liệu đơn đã thay đổi bởi thao tác khác. Vui lòng tải lại trước khi lưu.");throw r.status=409,r.code="ORDER_VERSION_CONFLICT",r}
function Ne(e={},r={}){
...
;return t.some(e=>a.has(e))}function Ae(e=[]){const r=new Map;for(const t of e||[]){const e=S(t.status)
...
function we(r={},t={}){const n=h(r.status||r.returnStatus||"active"),a={returnOrderId:h(r.id||r._id),returnOrderCode:h(r.code||r.id),
...
amount:e(r.totalAmount||r.amount||r.totalReturnAmount||r.debtReduction)}]}function Ie(r={},n=[]){
const o=Ae(n),d=o.reduce((r,t)=>r+e(t.returnAmount||t.amount),0),s=t.buildCanonicalDeliveryOrder(r,{returnItems:o,returnAmountOverride:d}),i=s.amounts||{};return{...s,orderId:X(r),
...
processed:e(i.processed),debt:a(i.debt??i.debtAmount)},reconciliation:be(i),status:{deliveryStatus:h(r.deliveryStatus||r.status||"pending"),
...
accountingStatus:h(r.accountingStatus||"")}}}function be(r={}){
...
message:Math.abs(u)<=1e3?"Đối soát OK":`Chênh lệch ${u.toLocaleString("vi-VN")}`}}function De(r=[]){return r.reduce((r,t)=>{const n=t.amounts||{}
...
reward:0,returnAmount:0,debt:0})}function $e(e={}){return S((e.status&&"object"==typeof e.status?e.status:{}).deliveryStatus||e.deliveryStatus||e.status||"pending")}
function Re(e={}){return["delivered","success","done","completed"].includes($e(e))}function ke(r=[],t={}){
const n=S(t.statusFilter||t.deliveryStatusFilter||t.orderStatusFilter||"all")
;return!n||["all","tat ca","tất cả","*"].includes(n)?r:["delivered","da giao","đã giao"].includes(n)?r.filter(Re):["pending","not_delivered","not-delivered","chua giao","chưa giao"].includes(n)?r.filter(e=>!Re(e)):["return","returns","has_return","tra hang","trả hàng"].includes(n)?r.filter(r=>e(r.amounts&&r.amounts.returnAmount)>0||e(r.returnAmount||r.returnTotal||r.totalReturnAmount)>0):["debt","cong no","công nợ"].includes(n)?r.filter(e=>a((e.amounts&&e.amounts.debt)??e.debtAmount??e.debt)>0):r
}class Me{constructor(e={}){this.SalesOrder=e.SalesOrder,this.MasterOrder=e.MasterOrder,this.ReturnOrder=e.ReturnOrder,this.StockTransaction=e.StockTransaction,
...
const r=h(e.date||e.deliveryDate||g()),t=N(e.status||e.deliveryStatus),n=N(e.q||e.keyword),a=()=>{const n={};return r&&(n.deliveryDate=r),
...
(await s.lean()).map(F).filter(Q)}async getCanonicalOrderByKey(e,r={}){const t=Oe(e);if(!t)return null;let n=this.SalesOrder.findOne(t);n=fe(n,r.session);const a=await n.lean()
;if(!a)return null;const o=await this.findReturnOrdersFor([a],r);return Ie(a,o.filter(e=>Ne(e,a)))}async listOrders(e={}){
const r=te(await this.findOrders(e)),t=await this.findReturnOrdersFor(r);let n=r.map(e=>Ie(e,t.filter(r=>Ne(r,e))));return n=te(ke(n,e)),
(w(e.checkStaffAssignment)||w(e.checkStaff)||w(e.staffCheck))&&(n=await this.enrichStaffAssignment(n)),{rows:n,summary:De(n),reconciliation:this.reconcileRows(n)}}
...
async saveReturn(r={}){const t=arguments[1]||{},n=h(r.salesOrderId||r.orderId||r.salesOrderCode||r.orderCode);let a=this.SalesOrder.findOne(Oe(n));a=fe(a,t.session)
;const o=await a.lean();if(!o){const e=new Error("Không tìm thấy đơn giao hàng");throw e.status=404,e}Se(o,r)
...
deliveryDate:h(o.deliveryDate||r.deliveryDate||g()),date:h(r.date||o.deliveryDate||g()),documentDate:h(r.documentDate||r.date||o.deliveryDate||g()),
...
const e=new Error(u.error);throw e.status=u.status||400,e}const c=u&&u.returnOrder||u,f=await this.getCanonicalOrderByKey(X(o),t),m=we(c,f||o);return{order:f,returnOrder:c,
...
;throw e.status=404,e}Se(a,r);const o=b(a),d=I(a);if(o&&!d){const e=new Error("Đơn đã xác nhận kế toán, cần mở khóa admin trước khi sửa tiền");throw e.status=423,e}
...
},C=pe(await this.SalesOrder.findOneAndUpdate(ge(n,a),{$set:y,$inc:{version:1}},{new:!0,lean:!0,session:t.session}));return{order:await this.getCanonicalOrderByKey(X(C),t),
...
;if(!n){const e=new Error("Không tìm thấy đơn giao hàng");throw e.status=404,e}if(Se(n,e),n.reconciliation&&!n.reconciliation.balanced){
...
},s=pe(await this.SalesOrder.findOneAndUpdate(ge(t,n),{$set:d,$inc:{version:1}},{new:!0,lean:!0,session:r.session}));return{order:await this.getCanonicalOrderByKey(X(s),r),
message:"Đã xác nhận giao hàng"}}reconcileRows(e=[]){const r=De(e),t=Math.round(r.receivable-r.cash-r.bank-r.reward-r.returnAmount-r.debt);return{...r,difference:t,
...
}).skip(l).limit(i).lean()).map(F).filter(r=>"1"===String(e.includeZeroValue??e.showZero??"0")||Q(r)),c=u.flatMap(e=>we(e,{}));return{returnOrders:u,returns:u,rows:c,summary:x(c)}}
async listReturns(e={}){const r=O([e.salesOrderId,e.orderId,e.salesOrderCode,e.orderCode,e.orderKey]);let t=null,n=[];if(r.length){const a=[],o=O(r.flatMap(k))
...
code:e});const d=a.length?(await this.ReturnOrder.find({...ue(),$or:a}).lean()).map(F).filter(Q):[],s=he(d,e);if(d.length&&!s.length&&ye(e))return{rows:[],returnOrdersRaw:[],
summary:x([])};if(s.length){let e={};for(const t of r)if(e=await this.getCanonicalOrderByKey(t)||{},e&&(e.orderId||e.orderCode))break;const t=s.flatMap(r=>we(r,e));return{rows:t,
returnOrdersRaw:s,summary:x(t)}}for(const t of r){const r=await this.getCanonicalOrderByKey(t);if(r&&ve(r,e)){n=[r];break}}if(!n.length&&ye(e))return{rows:[],returnOrdersRaw:[],
summary:x([])};t={rows:n}}else{const r=await this.listReturnDocuments(e);if((r.rows||[]).length||e.deliveryStaffCode||e.delivery||e.date||e.deliveryDate)return{rows:r.rows||[],
returnOrdersRaw:r.returnOrders||[],summary:r.summary||x(r.rows||[])};t=await this.listOrders(e),n=t.rows||[]}const a=new Map,o=new Map;for(const e of n||[]){
for(const r of O([e.orderId,e.salesOrderId,e.id]))a.set(r,e);for(const r of O([e.orderCode,e.salesOrderCode,e.code]))o.set(r,e)}const d=await this.findReturnOrdersFor(n),s=[]
;for(const e of d||[]){
...
;s.push(...we(e,r))}return{rows:s,returnOrdersRaw:d.map(F),summary:x(s)}}async reconciliation(e={}){return(await this.listOrders(e)).reconciliation}}function _e(e={}){return e}
module.exports={DeliveryEngine:Me,buildDeliveryAssignment:_e,buildCanonicalOrder:Ie,buildOrderReconciliation:be,summarizeOrders:De,helpers:{text:h,unique:O,orderIdOf:X,
orderCodeOf:J,productCodeOf:ne,returnMatchesOrder:Ne,buildOrderLookup:Oe,canonicalizeReturnDocument:F,summarizeReturnRows:x}};
