...
// ===== SCOPED FIX: AR_RETURN_RETURNORDER_CODE_MATCH_START =====
function normalizeReturnOrderLookupCode(value) {
  return String(value || '').trim().replace(/^RO[-_\s]*/i, '');
}

function buildReturnOrderCodeVariants(codes = []) {
  const variants = [];
  for (const raw of codes || []) {
    const value = String(raw || '').trim();
    if (!value) continue;
    const normalized = normalizeReturnOrderLookupCode(value);
    variants.push(value);
    if (normalized) {
      variants.push(normalized);
      variants.push(`RO-${normalized}`);
    }
  }
  return [...new Set(variants.filter(Boolean))];
}

function returnOrderCodeMatches(rowCode, orderCodeSet = new Set(), normalizedOrderCodeSet = new Set()) {
  const value = String(rowCode || '').trim();
  if (!value) return false;
  return orderCodeSet.has(value) || normalizedOrderCodeSet.has(normalizeReturnOrderLookupCode(value));
}
// ===== SCOPED FIX: AR_RETURN_RETURNORDER_CODE_MATCH_END =====
...
async function findReturnOrdersForDeliveryChildren(children = []) {
  ...
  const returnOrderCodeVariants = buildReturnOrderCodeVariants(orderCodes);
  ...
  if (returnOrderCodeVariants.length) {
    // returnOrders lưu từ DeliveryEngine có thể dùng code/id dạng RO-HUxxxx,
    // trong khi salesOrders dùng HUxxxx. Mở rộng lookup theo cả 2 dạng.
    or.push(
      { id: { $in: returnOrderCodeVariants } },
      { code: { $in: returnOrderCodeVariants } },
      { returnOrderCode: { $in: returnOrderCodeVariants } }
    );
  }
  if (masterIds.length) {
  ...
  debugLog('DEBUG_AR_RETURN', '[AR_RETURN_DEBUG] RETURN_QUERY', JSON.stringify({
    orderIds,
    orderCodes,
    returnCollection: typeof returnOrderRepository.collectionName === 'function' ? returnOrderRepository.collectionName() : undefined,
    masterIds,
    masterCodes,
    returnOrderCodeVariants,
    query
  }, null, 2));

  const projection = {
    id: 1, code: 1, returnOrderCode: 1, salesOrderId: 1, salesOrderCode: 1, orderId: 1, orderCode: 1,
    ...
    items: 1, status: 1, returnStatus: 1, accountingConfirmed: 1, accountingStatus: 1, returnMergeStatus: 1, warehouseReceiveStatus: 1,
  ...
  debugLog('DEBUG_AR_RETURN', '[AR_RETURN_DEBUG] RETURN_ROWS_RAW', {
    count: rows.length,
    rows: rows.map((row) => ({
      id: row.id,
      code: row.code,
  ...
  if (!rows.length) {
    ...
    if (fallbackAnd.length) {
      ...
      const orderCodeSet = new Set([...orderCodes, ...returnOrderCodeVariants]);
      const normalizedOrderCodeSet = new Set([...orderCodes, ...returnOrderCodeVariants].map(normalizeReturnOrderLookupCode).filter(Boolean));
      rows = (fallbackRows || []).filter((row) => {
        ...
        const rowCodes = [row.id, row.code, row.returnOrderCode, row.orderCode, row.salesOrderCode, row.sourceOrderCode, row.deliveryOrderCode].map((value) => String(value || '').trim()).filter(Boolean);
        return rowIds.some((value) => orderIdSet.has(value)) || rowCodes.some((value) => returnOrderCodeMatches(value, orderCodeSet, normalizedOrderCodeSet));
      });
    }
  }
