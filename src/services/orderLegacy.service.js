...
function T(e,t=O){return h(e||t)}function _(e={}){return e.saleMethod??e.saleMode??e.pricingMode??e.orderPricingMode??e.priceMode}function E(e={}){const t=_(e)
...
promotionName:String(t.promotionName||t.name||t.programName||t.ruleName||t.description||"").trim()}}function H(e={}){return E(e)?T(_(e))===O:U(e)}function z(e=[],t=O,r={}){
const o=T(t),a=H({...r,saleMode:o});return(Array.isArray(e)?e:[]).map(e=>{
const t=a?O:T(e.saleMethod||e.saleMode||e.pricingMode,o),r=f(e.quantity??e.qty??e.totalQty),n=f(e.unitPrice??e.price??e.salePrice??e.finalPrice),i=f(e.amount||r*n),s=f(e.originalPrice||e.grossPrice||e.catalogSalePrice||n),d=Math.round(r*s),c=Math.max(0,f(e.discountAmount||e.totalDiscountAmount||e.promotionAmount||d-i)),u=s>0&&r>0?c/d*100:f(e.discountPercent||0),l=Array.isArray(e.promotionRows)?e.promotionRows:[],m=x(l)
...
lockedPrice:a||t===N,lockedPromotion:t===N,promotionRows:l,...m}}).filter(e=>e.quantity>0||e.productCode||e.productName)}async function F(e=[],t=O,r={}){const o=T(t);if(H({...r,
saleMode:o}))return z(e,O,{...r,saleMode:O});const a=z(e,N,{...r,saleMode:N});if(!a.length)return a;if(B(a))return a;const n=await I.calculatePromotions(a,{
customerCode:r.customerCode||r.customer?.code||r.customer?.customerCode,date:r.date||r.orderDate||r.saleDate}),i=new Map((n.lines||[]).map(e=>[String(e.productCode||"").trim(),e]))
;return a.map(e=>{
...
salesStaffId:r?.id||"",salesStaffCode:r?.code||c(e),salesStaffName:r?.name||u(e)}}async function j(e,t=O){
...
saleMode:o,pricingMode:o,priceLocked:Boolean(e.priceLocked)||o===N||o===O}})}function G(e={},t=""){const o=r.nowIso();return{...e,stockPosted:!0,stockPostedAt:e.stockPostedAt||o,
...
async function Y(e,t={}){await C.postSaleOut(e,t)}async function J(e,t={}){await C.reverseMovement(e,{type:"SALE",reverseType:"SALE_REVERSAL",direction:"OUT",refType:"SALES_ORDER",
refId:e.id||e._id||e.code,refCode:e.code||e.id,date:r.todayVN(),note:"Đảo xuất kho đơn bán"},t)}async function ee(e,t={}){X(e)&&await v.reverseSalesOrderAR(e,t)}function te(e){
const t=g(e),o=D.lifecyclePatch(e,{source:t}),a=D.normalizeMergeStatus({...e,...o}),n=r.toDateOnly(e.orderDate||e.date||e.documentDate||e.importDate||e.displayDate||"");return{
...
salesStaffName:e.salesStaffName||e.salesPersonName||e.salesmanName||e.nvbhName||e.maNVBHName||"",visibleInHistory:D.isOrderVisibleInHistory({...e,...o})}}async function re(e){
...
$or:me.flatMap(e=>ge(e,r))}:{salesStaffCode:r}:null}function Ce(e={}){return me.map(t=>pe(e,t)).map(Se).filter(Boolean)}function ve(e={},t=""){const r=Se(t)
;return!r||Ce(e).some(e=>e===r)}function Ae(e={}){return e.salesStaffCode||e.salesPersonCode||e.salesmanCode||e.nvbhCode||e.maNVBH||pe(e,"salesStaff.code")||""}function Ie(e={}){
return e.salesStaffName||e.salesPersonName||e.salesmanName||e.nvbhName||e.maNVBHName||pe(e,"salesStaff.name")||pe(e,"salesStaff.fullName")||""}function De(e={}){
...
;const p=String(t.source||t.orderSource||"").trim(),g=D.normalizeOrderSource(p);if(g&&"manual"!==g){
...
const y=String(t.deliveryStatus||"").trim();y&&(d.deliveryStatus=y);const C=String(t.accountingStatus||"").trim();C&&(d.accountingStatus=C)
;const v=String(t.status||t.lifecycleStatus||"").trim();if(v&&"cancelled"!==v&&(d.status=v),o){const e=S.buildRegex(o);ue(c,{$or:/^[A-Z0-9_-]{5,}$/i.test(o)?[{code:o},{id:o},{
...
strictSalesStaffCode:m}}function Oe(e={}){const t=g(e),o=D.normalizeMergeStatus(e);return{id:e.id||e.code,code:e.code||e.orderCode||e.salesOrderCode||e.id,
...
visibleInHistory:D.isOrderVisibleInHistory(e)}}const Ne={_id:0,id:1,code:1,documentCode:1,invoiceCode:1,orderCode:1,salesOrderCode:1,date:1,orderDate:1,deliveryDate:1,createdAt:1,
...
paidAmount:1,debtAmount:1,amount:1,total:1};async function he(e={}){const t=Date.now(),{filter:r,guardedQuery:a,strictSalesStaffCode:n}=De(e),i=S.getPagination(a,{defaultLimit:50,
...
})),[{orders:m,queryMs:f},{total:p,countMs:g}]=await Promise.all([c,l]),y=Date.now(),C=m.map(Oe),v=Date.now()-y,A=Date.now()-t
;return w("DEBUG_ORDER_FLOW","[SALES_ORDER_SEARCH_REBUILT]",{ms:A,queryMs:f,countMs:g,mapMs:v,page:i.page,limit:i.limit,total:p,returned:C.length,filter:r}),{rows:C,salesOrders:C,
orders:C,total:p,page:i.page,limit:i.limit,returned:C.length,hasMore:i.skip+C.length<p,ms:A,queryMs:f,countMs:g,mapMs:v}}function we(e={}){
...
;const C=[Pe(m,l),Me(t.status||t.lifecycleStatus),Re(t.mergeStatus),$e(t.deliveryStatus),Be(t.accountingStatus)].filter(Boolean);return C.length&&we(f).push(...C),
(await o.findAll(f,{projection:Ne,sort:{createdAt:-1,code:-1},skip:n.skip,limit:n.limit})).map(te)}async function ke(e={},t={}){
const a=Date.now(),n=await Q(e),i=await Z(e),s=E(e)?T(_(e)):U(e)?O:N,d=await j(await F(e.items,s,e),s);if(!d.length)return{error:"Đơn bán chưa có sản phẩm",status:400}
;const c=f(e.totalAmount||d.reduce((e,t)=>e+f(t.amount),0)),u=f(e.paidAmount||e.paid||0),l=String(e.code||e.orderCode||e.salesOrderCode||e.documentCode||L()).trim(),m=String(e.id||l).trim(),S=r.toDateOnly(e.orderDate||e.date||r.todayVN()),g=r.toDateOnly(e.deliveryDate||S),C=K(e,n,i),v=String(t.role||"").trim().toLowerCase(),A=!["admin","accountant"].includes(v)||k(e.vatInvoiceRequired,!0),I=A?"default":"manual",h=String(t.username||t.name||t.fullName||t.code||e.updatedBy||e.userName||"").trim(),P={
...
customerAddress:n?.address||e.customerAddress||"",staffId:C.salesStaffId,staffCode:C.salesStaffCode,staffName:C.salesStaffName,salesStaffId:C.salesStaffId,
salesStaffCode:C.salesStaffCode,salesStaffName:C.salesStaffName,salesmanCode:e.salesmanCode||C.salesStaffCode,salesmanName:e.salesmanName||C.salesStaffName,saleMethod:s,saleMode:s,
pricingMode:s,orderPricingMode:s,isPromotionSale:s===N,priceLocked:s===O,promotionCalculated:s===N,items:d,totalAmount:c,paidAmount:u,debtAmount:f(e.debtAmount??Math.max(0,c-u)),
...
;Object.assign(P,D.lifecyclePatch(P,{source:e.source||e.orderSource||"sales_app"})),Object.assign(P,y(P));const M=G(P,e.createdBy||e.userName||e.staffName||"create_order")
...
;const n=E(t),i=!n&&(H(a)||H(t)),s=n?T(_(t)):i?O:T(a.saleMethod||a.saleMode||a.pricingMode||a.orderPricingMode||N),d=t.items?await j(await F(t.items,s,{...a,...t,saleMode:s
}),s):a.items,c=f(t.totalAmount??(d||[]).reduce((e,t)=>e+f(t.amount),0)),u=f(t.paidAmount??a.paidAmount??0),m=r.toDateOnly(t.orderDate||t.date||a.orderDate||a.date||r.todayVN()),S=r.toDateOnly(t.deliveryDate||a.deliveryDate||m),g=K(t,null,null,a),v=y({
...a,...t,date:m,orderDate:m,documentDate:m,deliveryDate:S,saleMethod:s,saleMode:s,pricingMode:s,orderPricingMode:s,isPromotionSale:s===N,priceLocked:i||s===O,
...
...D.lifecyclePatch({...a,...t,items:d,totalAmount:c,paidAmount:u},a),updatedAt:r.nowIso()});let I=v;return await p.withMongoTransaction(async e=>{
const r=W(a),n=!0===t.postImmediately||r,i=n?G(v,t.updatedBy||t.userName||"update_order"):v;if(I=i,await o.upsert(i,{session:e}),await A.syncReturnDraftWithSalesOrder(i,{session:e
}),r&&n){const t=Ee(a.items,i.items);if(t.hasDelta){const r=l("SOEDIT");t.inItems.length&&await C.postSaleEditDelta(i,t.inItems,"IN",{session:e,commandId:`${r}:IN`}),
t.outItems.length&&await C.postSaleEditDelta(i,t.outItems,"OUT",{session:e,commandId:`${r}:OUT`})}}else!r&&n&&await Y(i,{session:e})}),{salesOrder:te(I)}}
