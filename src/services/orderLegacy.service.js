...
"use strict"
;const e=require("mongoose"),t=require("../utils/deliveryFinance.util"),r=require("../utils/date.util"),o=require("../repositories/orderRepository"),a=require("../repositories/masterOrderRepository"),n=require("../repositories/productRepository"),i=require("../repositories/customerRepository"),s=require("../repositories/userRepository"),d=require("../rules/staffRules"),{pickSalesStaffCode:c,pickSalesStaffName:u}=require("../domain/staff/staffIdentity"),{makeId:l,normalizeText:m,toNumber:f}=require("../utils/common.util"),S=require("../utils/queryGuard.util"),p=require("../utils/transaction.util"),{normalizeOrderSourceValue:g,applyOrderSourceFields:y}=require("../utils/orderSource.util"),v=require("../domain/posting/InventoryPostingService"),C=require("../engines/posting.engine"),A=require("./returnOrderService"),I=require("./promotionService"),O=require("../utils/orderStatus.util"),{DIRECT_PRICE:D,PROMOTION:N,normalizePricingMode:w}=require("../constants/pricingModes"),{debugLog:h}=require("../utils/debug.util"),{normalizePickingZone:P,pickingZoneFrom:M,legacyPrintGroupCode:R,PICKING_ZONES:$}=require("../utils/pickingZone.util")
;function B(t=[]){return!(!e.connection||1===e.connection.readyState)&&(t||[]).every(e=>void 0!==e.price||void 0!==e.salePrice||void 0!==e.unitPrice)}function b(e){
return r.toDateOnly(e)}function k(e,t=!0){return!1!==e&&"false"!==String(e).trim().toLowerCase()&&(!0===e||"true"===String(e).trim().toLowerCase()||!1!==t)}function q(e){
...
function T(e,t=D){return w(e||t)}function _(e={}){return e.saleMethod??e.saleMode??e.pricingMode??e.orderPricingMode??e.priceMode}function E(e={}){const t=_(e)
...
promotionRows:m,...S}})}async function Z(e={}){const t=String(e.customerId||e.customerCode||e.customerName||"").trim();return t?i.findByIdOrCode(t):null}async function K(e={}){
const t=String(e.salesStaffCode||e.salesmanCode||"").trim();return t?d.resolveSalesStaffByCode(t):null}function j(e={},t=null,r=null,o=null){
...
salesStaffId:r?.id||"",salesStaffCode:r?.code||c(e),salesStaffName:r?.name||u(e)}}async function Q(e,t=D){
...
paidAmount:1,debtAmount:1,amount:1,total:1};async function we(e={}){const t=Date.now(),{filter:r,guardedQuery:a,strictSalesStaffCode:n}=Oe(e),i=S.getPagination(a,{defaultLimit:50,
...
;return h("DEBUG_ORDER_FLOW","[SALES_ORDER_SEARCH_REBUILT]",{ms:A,queryMs:f,countMs:g,mapMs:C,page:i.page,limit:i.limit,total:p,returned:v.length,filter:r}),{rows:v,salesOrders:v,
orders:v,total:p,page:i.page,limit:i.limit,returned:v.length,hasMore:i.skip+v.length<p,ms:A,queryMs:f,countMs:g,mapMs:C}}function he(e={}){
...
$nin:["confirmed","locked","posted"]}}]}:{accountingStatus:t}:null}async function be(e={}){const t=S.normalizeQueryDateRange(e,{defaultToday:!0
...
}]:t}if(u||ce(f),i){const e=S.buildRegex(i);he(f).push({$or:[{code:e},{id:e},{orderCode:e},{salesOrderCode:e},{customerCode:e},{customerName:e},{customerPhone:e},{salesStaffCode:e
},{salesStaffName:e},{deliveryStaffCode:e},{deliveryStaffName:e},{masterOrderCode:e},{masterOrderId:e}]})}
const p=q(t.salesStaffCode||t.salesmanCode||t.nvbhCode||t.maNVBH),g=String(t.salesStaffText||t.salesStaffName||t.salesmanName||"").trim();if(p)f.salesStaffCode=p;else if(g){
const e=S.buildRegex(g);he(f).push({$or:[{salesStaffCode:e},{salesStaffName:e}]})}const y=q(t.deliveryStaffCode||t.nvghCode||t.deliveryCode);y&&(f.deliveryStaffCode=y)
;const v=[Pe(m,l),Me(t.status||t.lifecycleStatus),Re(t.mergeStatus),$e(t.deliveryStatus),Be(t.accountingStatus)].filter(Boolean);return v.length&&he(f).push(...v),
(await o.findAll(f,{projection:Ne,sort:{createdAt:-1,code:-1},skip:n.skip,limit:n.limit})).map(te)}async function ke(e={},t={}){
const a=Date.now(),n=await Z(e),i=await K(e),s=E(e)?T(_(e)):U(e)?D:N,d=await Q(await F(e.items,s,e),s);if(!d.length)return{error:"Đơn bán chưa có sản phẩm",status:400}
;const c=f(e.totalAmount||d.reduce((e,t)=>e+f(t.amount),0)),u=f(e.paidAmount||e.paid||0),l=String(e.code||e.orderCode||e.salesOrderCode||e.documentCode||L()).trim(),m=String(e.id||l).trim(),S=r.toDateOnly(e.orderDate||e.date||r.todayVN()),g=r.toDateOnly(e.deliveryDate||S),v=j(e,n,i),C=String(t.role||"").trim().toLowerCase(),A=!["admin","accountant"].includes(C)||k(e.vatInvoiceRequired,!0),I=A?"default":"manual",w=String(t.username||t.name||t.fullName||t.code||e.updatedBy||e.userName||"").trim(),P={
...
vatInvoiceUpdatedBy:"manual"===I?w||"system":"",arStatus:e.arStatus||"pending",arBalance:0,createdAt:e.createdAt||r.nowIso(),updatedAt:r.nowIso()}
...
returnAmount:f(M.returnAmount||0)},{session:e}),await Y(M,{session:e})}),h("DEBUG_ORDER_FLOW","[CREATE_ORDER_DONE]",{ms:Date.now()-a,code:M.code,itemCount:d.length,stockPosted:!0
}),{salesOrder:te(M)}}async function qe(e,t={}){const a=await o.findByIdOrCode(e);if(!a)return{error:"Không tìm thấy đơn bán",status:404}
;if(a.masterOrderId||"merged"===a.mergeStatus)return{error:"Đơn đã gộp, không nên sửa trực tiếp đơn con",status:400}
;const n=E(t),i=!n&&(H(a)||H(t)),s=n?T(_(t)):i?D:T(a.saleMethod||a.saleMode||a.pricingMode||a.orderPricingMode||N),d=t.items?await Q(await F(t.items,s,{...a,...t,saleMode:s
}),s):a.items,c=f(t.totalAmount??(d||[]).reduce((e,t)=>e+f(t.amount),0)),u=f(t.paidAmount??a.paidAmount??0),l=r.toDateOnly(t.orderDate||t.date||a.orderDate||a.date||r.todayVN()),m=r.toDateOnly(t.deliveryDate||a.deliveryDate||l),S=j(t,null,null,a),g=y({
...a,...t,date:l,orderDate:l,documentDate:l,deliveryDate:m,saleMethod:s,saleMode:s,pricingMode:s,orderPricingMode:s,isPromotionSale:s===N,priceLocked:i||s===D,
...
vatInvoiceUpdatedBy:String(a.vatInvoiceUpdatedBy||""),salesStaffId:S.salesStaffId,salesStaffCode:S.salesStaffCode,salesStaffName:S.salesStaffName,
salesmanCode:a.salesmanCode||S.salesStaffCode,salesmanName:a.salesmanName||S.salesStaffName,staffId:S.salesStaffId,staffCode:S.salesStaffCode,staffName:S.salesStaffName,
...O.lifecyclePatch({...a,...t,items:d,totalAmount:c,paidAmount:u},a),updatedAt:r.nowIso()});let v=g;return await p.withMongoTransaction(async e=>{
const r=W(a),n=!0===t.postImmediately||r;r&&await J(a,{session:e});const i=n?G(g,t.updatedBy||t.userName||"update_order"):g;v=i,await o.upsert(i,{session:e}),
await A.syncReturnDraftWithSalesOrder(i,{session:e}),n&&await Y(i,{session:e})}),{salesOrder:te(v)}}async function Le(e,t={},a={}){const n=await o.findByIdOrCode(e);if(!n)return{
error:"Không tìm thấy đơn bán",status:404};if("boolean"!=typeof t.vatInvoiceRequired)return{error:"Thiết lập hóa đơn VAT không hợp lệ",status:400};const i=r.nowIso(),s={
vatInvoiceRequired:t.vatInvoiceRequired,vatInvoiceDecisionSource:"manual",vatInvoiceNote:String(t.note||t.vatInvoiceNote||"").trim(),vatInvoiceUpdatedAt:i,
vatInvoiceUpdatedBy:String(a.username||a.name||a.fullName||a.code||"system").trim(),updatedAt:i};return{salesOrder:te(await o.patchByIdentity(n.id||n.code||e,s)||{...n,...s})}}
async function Te(e,t={}){const a=await o.findByIdOrCode(e);if(!a)return{error:"Không tìm thấy đơn bán",status:404};const n=await A.cancelReturnDraftForSalesOrder(a,{dryRun:!0})
;if(n&&n.error)return n;const i=r.nowIso(),s=W(a),d={status:"cancelled",deliveryStatus:"cancelled",cancelReason:String(t.reason||t.cancelReason||"").trim(),cancelledAt:i,
updatedAt:i,...s?{stockPosted:!1,stockReversedAt:i}:{}};let c={...a,...d};return await p.withMongoTransaction(async t=>{s&&await J(a,{session:t}),await ee(a,{session:t}),
await A.cancelReturnDraftForSalesOrder(a,{session:t});const r=await o.patchByIdentity(a.id||a.code||e,d,{session:t});r&&(c=r)}),
(c.masterOrderId||c.masterOrderCode)&&await ze(c.masterOrderId||c.masterOrderCode),{salesOrder:te(c)}}async function _e(e,t={}){const a=await o.findByIdOrCode(e);if(!a)return{
error:"Không tìm thấy đơn bán",status:404};const n=await A.cancelReturnDraftForSalesOrder(a,{dryRun:!0});if(n&&n.error)return n;const i=W(a),s={...a,status:"void",
deliveryStatus:"void",deleted:!0,isDeleted:!0,deletedAt:r.nowIso(),deleteReason:String(t.reason||t.deleteReason||"").trim(),updatedAt:r.nowIso()}
;return await p.withMongoTransaction(async e=>{await o.upsert(s,{session:e}),await A.cancelReturnDraftForSalesOrder(a,{session:e}),i&&await J(a,{session:e}),await ee(a,{session:e})
}),(s.masterOrderId||s.masterOrderCode)&&await ze(s.masterOrderId||s.masterOrderCode),{hardDeleted:!1,salesOrder:te(s)}}function Ee(e={}){
return[e.id,e.code,e.orderNo,e.orderCode,e._id].map(e=>String(e||"").trim()).filter(Boolean)}function Ue(e={}){const t=String(e.status||"").toLowerCase()
;return["cancelled","canceled","void","deleted","removed"].includes(t)||Boolean(e.deletedAt)}function Ve(e={}){
return new Set((Array.isArray(e.childOrderIds)?e.childOrderIds:[]).map(e=>String(e?.id||e?.code||e?._id||e||"").trim()).filter(Boolean))}async function xe(e={}){const t=Ve(e)
;if(!t.size)return[];const r=await o.findManyByIdentity(Array.from(t)),a=new Map;for(const e of r){if(Ue(e))continue;const r=Ee(e).some(e=>t.has(e));if(!r)continue
;const o=String(e.id||e.code||e._id||"").trim();o&&a.set(o,e)}return Array.from(a.values())}function He(e=[]){
const r=e.filter(e=>!Ue(e)),o=r.length,a=r.reduce((e,t)=>e+(Array.isArray(t.items)?t.items:[]).reduce((e,t)=>e+f(t.quantity??t.qty??t.totalQuantity??0),0),0),n=r.reduce((e,t)=>e+f(t.totalAmount),0),i=r.reduce((e,t)=>e+f(t.paidAmount),0),s=r.reduce((e,r)=>e+t.calculateDeliveryDebt(r),0)
;return{orderCount:o,totalOrders:o,totalQuantity:a,totalAmount:n,paidAmount:i,debtAmount:s,totalDebt:s}}async function ze(e,t={}){const o=await a.findByIdOrCode(e)
;if(!o)return null;const n=await xe(o),i=n.map(e=>e.id||e.code).filter(Boolean),s={...o,childOrderIds:i,children:[],...He(n),updatedAt:r.nowIso()};return await a.upsert(s,t),s}
module.exports={listOrders:be,searchOrders:we,getOrder:re,createOrder:ke,updateOrder:qe,updateVatInvoiceSetting:Le,cancelOrder:Te,deleteOrder:_e,getMasterChildren:xe,
summarizeOrders:He,syncMasterOrderSummary:ze,applySalesOrderPosting:Y,reverseSalesOrderPosting:J,toClient:te};
