...
function buildReturnARLedgerEntry(returnOrder = {}, options = {}) {
  ...
  const entry = {
    ...
    accountingConfirmedBy: cleanString(options.confirmedBy || options.user?.code || options.user?.name || options.user || returnOrder.accountingConfirmedBy || returnOrder.createdBy || 'system'),
    debit: 0,
    credit: amount,
    amount,
    amountField: 'credit',
    amountSourceField: validation.amountField,
