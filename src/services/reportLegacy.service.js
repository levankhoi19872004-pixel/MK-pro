...
return f(e.totalAmount??e.amount??e.grandTotal??e.total??e.value)}function D(e=[],t=R){return e.reduce((e,o)=>e+f(t(o)),0)}
const N=["void","cancelled","canceled","deleted","duplicate_cancelled"];function I(e,t,o={}){"test"!==process.env.NODE_ENV&&console.error("[REPORT_DATA_SOURCE_FAILED]",{report:t,
...
async function T(e,t,o){try{return await o()}catch(o){throw I(o,e,t)}}function w(e={},t=50,o=200){const r=ce(e.page),n=se(e.limit,t,o);return{page:r,limit:n,skip:(r-1)*n}}
function E(e,t,o){const r=Math.max(0,f(o));return{page:e,limit:t,total:r,totalPages:r>0?Math.ceil(r/t):0,hasMore:e*t<r}}function k(e={},t={},o=[]){
...
const n=m(r.q),a=Boolean(r.dateFrom||r.dateTo||r.asOfDate||"movement"===r.mode),d=["1","true","yes"].includes(String(r.full||r.export||"").toLowerCase()),{page:u,limit:i,skip:s}=w(r,50,200)
;if(a){const a=e.toDateOnly(r.dateFrom||"0000-01-01"),c=e.toDateOnly(r.dateTo||r.asOfDate||e.todayVN()),[l,p]=await T("tồn kho theo kỳ",r,()=>Promise.all([o.find(q({dateTo:c
},["date","createdAt"])).sort({date:1,createdAt:1,productCode:1}).lean(),t.find({}).lean()])),C=new Map(p.map(e=>[String(e.code||e.id||e._id),e])),$=new Map;l.forEach(t=>{
...
productId:t.productId||n.id||String(n._id||""),productCode:r,productName:t.productName||n.name||"",warehouseCode:y,warehouseName:g,unit:n.unit||t.unit||"",openingQty:0,importQty:0,
...
meta:d?E(1,Math.max(A.length,1),A.length):E(u,i,A.length),summary:h,negativeStockCount:b.length,negativeStockRows:b}}
const c=await T("tồn kho hiện tại",r,()=>p.getInventorySummary(r)),l=c.stock||[],C=d?l:l.slice(s,s+i);return{...c,source:"mongo_inventories_canonical",
inventorySource:"inventories",stock:C,items:C,meta:d?E(1,Math.max(l.length,1),l.length):E(u,i,l.length),summary:c.summary,negativeStockCount:c.negativeStockCount,
negativeStockRows:c.negativeStockRows}}async function V(t={}){
const{page:r,limit:n,skip:a}=w(t,50,200),d=k(U(t),t,["productCode","productName","warehouseCode","refCode","refType","type"]),u=Q(["quantity","qty"],0),i=await T("thẻ kho",t,()=>o.aggregate([{
...
source:"mongo_stock_transactions",transactions:c,items:c,meta:E(r,n,l.transactionCount||0),summary:{transactionCount:f(l.transactionCount),inQty:f(l.inQty),outQty:f(l.outQty)}}}
...
return t.length?1===t.length?t[0]:{$and:t}:null}function ye(e){return String(e||"").trim()}function ge(e=""){const t=ye(e).toUpperCase();if(!t)return""
;const o=t.match(/^RO-([A-Z0-9]+)$/i)||t.match(/^AR-RETURN:RO-([A-Z0-9]+)$/i)||t.match(/(?:^|[-_:])RO-([A-Z0-9]+)(?=$|[-_:])/i);return o?o[1]:""}function pe(e=[]){const t=new Set
;for(const o of e||[]){const e=ye(o);if(!e)continue;t.add(e);const r=ge(e);r&&t.add(r),/^[A-Z0-9]+$/i.test(e)&&!/^RO-/i.test(e)&&(t.add(`RO-${e}`),t.add(`AR-RETURN:RO-${e}`),
...
;const a=await T("phạm vi nhân viên công nợ",t,()=>d.find(n).select("orderId orderCode salesOrderId salesOrderCode sourceOrderId sourceOrderCode sourceId sourceCode returnOrderId returnOrderCode refId refCode id code idempotencyKey").limit(5e3).lean()),u=Array.from(new Set(a.flatMap(e=>[e.orderId,e.salesOrderId,e.sourceOrderId,e.sourceId,e.returnOrderId,e.refId]).map(ye).filter(Boolean))),i=pe(a.flatMap(e=>[e.orderCode,e.salesOrderCode,e.sourceOrderCode,e.sourceCode,e.returnOrderCode,e.refCode,e.idempotencyKey,e.code,e.id]).map(ye).filter(Boolean))
...
},{customerId:t},{type:t},{note:t}]})}const[a,u]=await T("sổ công nợ",e,()=>Promise.all([d.find(n).select(Ae).sort({date:-1,createdAt:-1}).skip(r).limit(o+1).lean(),d.aggregate([{
$match:n},{$group:{_id:null,arLedgerCount:{$sum:1},totalDebit:{$sum:Q(["debit"],0)},totalCredit:{$sum:Q(["credit"],0)}}}])])),i=u?.[0]||{},s=f(i.arLedgerCount);return{
source:"mongo_ar_ledger_fast",ledgerCollection:"arLedgers",debts:[],customerSummary:[],bySalesman:[],byDelivery:[],arLedger:(a.length>o?a.slice(0,o):a).map(de),arDiagnostics:[],
summary:{page:t,limit:o,hasMore:t*o<s,totalRows:s,arLedgerCount:s,totalDebit:f(i.totalDebit),totalCredit:f(i.totalCredit),totalDebt:f(i.totalDebit)-f(i.totalCredit),
arWarningCount:0,optimized:!0}}}function De(e=[],t={}){const o=t.codeKey||"salesmanCode",r=t.nameKey||"salesmanName",n=t.role||"person",a=new Map;return e.forEach(e=>{
const t=String(e[o]||"").trim(),d=String(e[r]||"").trim(),u=t||d||"UNASSIGNED";a.has(u)||a.set(u,{role:n,code:t,name:d||(t?"":"Chưa gán"),label:t&&d?`${t} - ${d}`:d||t||"Chưa gán",
...
maxAgingDays:0});const i=a.get(u),s=e.customerId||e.customerCode||e.customerName;s&&i.customerKeys.add(String(s)),i.orders+=1,"paid"===e.status&&(i.paidOrders+=1),
...
i.returnAmount+=f(e.returnAmount),i.bonusAmount+=f(e.bonusAmount),i.debt+=$(e.debt),i.maxOverdueDays=Math.max(i.maxOverdueDays,f(e.overdueDays)),
i.maxAgingDays=Math.max(i.maxAgingDays,f(e.agingDays))}),Array.from(a.values()).map(e=>{const{customerKeys:t,...o}=e;return{...o,customers:t.size,
collectionRate:o.debit>0?Math.round(o.credit/o.debit*1e4)/100:0,debt:Math.max(0,$(o.debt)),debtZeroTolerance:C,status:A(o.debt)?o.overdueOrders>0?"overdue":"open":"paid"}
}).sort((e,t)=>t.debt-e.debt||t.overdueOrders-e.overdueOrders||String(e.label).localeCompare(String(t.label)))}async function Ne(e={}){const t=await Se(e);return{source:t.source,
ledgerCollection:t.ledgerCollection,bySalesman:t.bySalesman,summary:t.summary}}async function Ie(e={}){const t=await Se(e);return{source:t.source,
ledgerCollection:t.ledgerCollection,byDelivery:t.byDelivery,summary:t.summary}}async function Te(t={}){
const{page:o,limit:n,skip:a}=w(t,50,200),d=k(x(t,["date","orderDate","documentDate","createdAt"]),t,["code","orderCode","customerCode","customerName","salesStaffCode","salesStaffName"]),u=Q(["totalAmount","amount","grandTotal","total","value"],0),i=Q(["paidAmount","paymentAmount"],0),s=L(["salesStaffCode","salesmanCode","nvbhCode"],""),c=L(["salesStaffName","salesmanName","nvbhName"],""),l=await T("bán hàng",t,()=>r.aggregate([{
...
}]).allowDiskUse(!0).exec()),m=l?.[0]||{},y=m.rows||[],g=Array.from(new Set(y.map(e=>String(e.customerCode||"").trim()).filter(Boolean))),p=await h.getCustomerDebtMap(g,{
status:"all"}),C=y.map(t=>{const o=p.get(String(t.customerCode||"").trim())||h._internal.emptyCustomerDebt(t.customerCode||"");return{id:t.id||String(t._id||""),
...
}),$=m.totals?.[0]||{},A=Array.from(p.values()).reduce((e,t)=>e+Number(t.currentDebtAmount||0),0);return{source:"mongo_aggregate",sales:C,items:C,meta:E(o,n,$.orderCount||0),
...
orderCount:f($.orderCount),totalAmount:f($.totalAmount),paidAmount:f($.paidAmount),debtAmount:A,debtSource:h.DEBT_SOURCE}}}async function we(e={}){
const{page:t,limit:o,skip:r}=w(e,50,200),n=x(e,["date","createdAt"]),d=x(e,["date","documentDate","createdAt"]),l=x(e,["date","documentDate","createdAt"]),m=x(e,["date","returnDate","documentDate","deliveryDate","createdAt"]),[y,g,p,C,$,A,b,h]=await T("tài chính",e,()=>Promise.all([a.find(d).sort({
...
},v=S("cash","in"),O=S("cash","out"),_=S("bank","in"),R=S("bank","out"),D=$?.[0]||{},N=A?.[0]||{},I={receipts:f(h?.[0]),cashbook:f(h?.[1]),bankbook:f(h?.[2]),returns:f(h?.[3])
},k=Math.max(...Object.values(I),0);return{source:"mongo_paged",fundSource:"fundLedgers",meta:{...E(t,o,k),categoryCounts:I},summary:{receiptCount:f(D.count),
totalReceipts:f(D.amount),cashIn:v,cashOut:O,cashBalance:v-O,bankIn:_,bankOut:R,bankBalance:_-R,totalFundIn:v+_,totalFundOut:O+R,totalFundBalance:v+_-O-R,returnCount:f(N.count),
totalReturns:f(N.amount)},receipts:y,cashbook:g,bankbook:p,returns:C}}async function Ee(t={}){
const{page:o,limit:r,skip:a}=w(t,50,200),d=k(x(t,["deliveryDate","date","createdAt"]),t,["code","masterOrderCode","deliveryStaffCode","deliveryStaffName","status"]),u=Q(["totalAmount","amount","grandTotal","total","value"],0),i=Q(["collectedAmount","paidAmount"],0),s={
...
}]).allowDiskUse(!0).exec()),y=m?.[0]||{},g=(y.rows||[]).map(t=>({id:t.id||String(t._id||""),code:t.code||t.masterOrderCode||"",
...
collectedAmount:f(t.collectedAmount||t.paidAmount),status:t.status||""})),p=y.totals?.[0]||{};return{source:"mongo_aggregate",delivery:g,items:g,meta:E(o,r,p.tripCount||0),
...
const t=Q(["totalAmount","amount","grandTotal","total","value"],0),o=Q(["paidAmount","paymentAmount"],0),a=x(e,["date","orderDate","documentDate","createdAt"]),d=x(e,["deliveryDate","date","createdAt"]),i=x(e,["date","createdAt"]),s=x(e,["date","documentDate","importDate","createdAt"]),[c,m,y,g,A,b]=await T("dashboard",e,()=>Promise.all([r.aggregate([{
$match:a},{$group:{_id:null,orderCount:{$sum:1},totalAmount:{$sum:t},paidAmount:{$sum:o}}}]),h.getDebtSummary({status:"all"}),p.getInventorySummary({}),u.aggregate([{$match:i},{
...
}])])),S=c?.[0]||{},v=m||{},O=A?.[0]||{},_=b?.[0]||{},R=(e,t)=>f((g||[]).find(o=>String(o?._id?.fundType||"").toLowerCase()===e&&String(o?._id?.direction||"").toLowerCase()===t)?.amount),D=R("cash","in"),N=R("cash","out"),I=R("bank","in"),w=R("bank","out"),E=$(f(v.totalDebt))
;return{source:"mongo_summary_only",dashboard:{sales:{orderCount:f(S.orderCount),totalAmount:f(S.totalAmount),paidAmount:f(S.paidAmount),debtAmount:E,debtSource:h.DEBT_SOURCE},
debts:{totalDebit:0,totalCredit:0,totalDebt:E,debtZeroTolerance:C},stock:y?.summary||{},finance:{cashIn:D,cashOut:N,cashBalance:D-N,bankIn:I,bankOut:w,bankBalance:I-w,
totalFundBalance:D+I-N-w},delivery:{tripCount:f(O.tripCount),totalAmount:f(O.totalAmount),collectedAmount:f(O.collectedAmount)},imports:{importCount:f(_.importCount),
...
debtBySalesmanReport:Ne,debtByDeliveryReport:Ie,dashboardReport:ke,salesReport:Te,financeReport:we,deliveryReport:Ee};
