...
async function calculate(req, res) { try { res.json({ ok: true, result: await promotionService.calculatePromotions(req.body?.items || []) }); } catch (err) { res.status(500).json({ ok: false, message: 'Không tính được khuyến mại', error: err.message }); } }

module.exports = { list, save, remove, listProductRules, saveProductRule, deleteProductRule, listGroupItems, saveGroupItem, deleteGroupItem, listGroupRules, saveGroupRule, deleteGroupRule, calculate };
