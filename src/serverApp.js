...
function normalizeData(data) {
  ...
  data.products = data.products.map((product) => ({
    ...
    packing: String(product.packing || product.package || product.packaging || '').trim(),
  ...
  data.importShortages = data.importShortages.map((row) => ({
    id: row.id || makeId('SH'),
    source: row.source || 'salesOrdersImport',
    ...
    importedQuantity: toNumber(row.importedQuantity),
    shortageQuantity: toNumber(row.shortageQuantity),
...
function pickProductPayload(body) {
  return {
    ...
    packing: String(body.packing || body.package || body.packaging || '').trim(),
