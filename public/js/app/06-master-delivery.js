...
function renderDeliveryEditTotal(){
  ...
  deliveryEditTotalBox.innerHTML=`<div class="delivery-total-chip total-receivable"><span>Phải thu</span><b>${money(state.before)}</b></div><div class="delivery-total-chip"><span>Tiền mặt</span><b>${money(state.cash)}</b></div><div class="delivery-total-chip"><span>Chuyển khoản</span><b>${money(state.bank)}</b></div><div class="delivery-total-chip"><span>Hàng trả</span><b>${money(state.returned)}</b></div><div class="delivery-total-chip"><span>Trả thưởng</span><b>${money(state.reward)}</b></div><div class="delivery-total-chip"><span>Đã nhập</span><b>${money(state.paid)}</b></div><div class="delivery-total-chip summary-inline-item total-debt"><span>Còn nợ</span><b>${money(state.debt)}</b></div><div class="delivery-total-chip summary-inline-item total-overpay"><span>Trả vượt</span><b>${money(state.over)}</b></div><div class="delivery-total-chip summary-inline-item total-match ${matched?'is-matched':'is-unmatched'}"><span>Đối soát</span><b>${matched?'Đã khớp':'Chưa khớp'}</b></div>`;
}
