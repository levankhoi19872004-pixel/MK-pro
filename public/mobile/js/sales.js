...
;import{buildCartItemsHtml as v,buildOrderCardsHtml as w,createMobileSalesNavigation as C,createStatusAnnouncer as k}from"./sales-ux.js?v=phase155-returns-modal-v1"
;import{collectMobileSalesDom as S}from"./sales/dom.js?v=phase86-production-hardening-v1"
;import{createMobileSalesState as P,OrderDraftStore as E}from"./sales/state.js?v=phase86-production-hardening-v1"
;import{buildDebtLookup as N,customerAddressValue as L,customerAvailableDebtValue as T,customerCodeValue as A,customerDebtValue as x,customerNameValue as $,customerPendingCollectedValue as D,customerPhoneValue as M,customerSalesValue as B,debtClassName as O,mergeCustomerDebt as I,mergeCustomerPages as q,normalizeSelectedCustomerForSubmit as K,uniqueCustomerIdentityKeys as Q}from"./sales/customer.js?v=phase86-production-hardening-v1"
;import{currentSalesStaffCode as _,filterOrdersForCurrentSalesUser as R}from"./sales/staff.js?v=phase86-production-hardening-v1"
;import{applyPromotionLines as U,attachPackingRate as H,buildPromotionCartPayloadItem as j,normalizePackingRate as F,normalizeProductGroupName as V,normalizeProductSearchResponse as G,toMobileProduct as z}from"./sales/product.js?v=phase86-production-hardening-v1"
...
;document.getElementById("staffInfo").textContent=`${le.name||le.username||"Nhân viên"}`;const ue=P({draftStore:new E({ownerKey:_(le)||le.id||le.username||"sales"})
}),{tabs:me,panels:he,customerSearch:ge,customerList:pe,customerLoadMoreBtn:be,productSearch:fe,productGroupFilter:ye,productSuggestions:ve,selectedCustomerBox:we,selectedProductBox:Ce,caseQtyInput:ke,looseQtyInput:Se,paidAmountInput:Pe,cartList:Ee,cartCustomerContext:Ne,cartCount:Le,cartTotal:Te,cartGrossTotal:Ae,cartDiscountTotal:xe,orderDraftBar:$e,orderDraftLineCount:De,orderDraftTotal:Me,openCartBtn:Be,backToOrderBtn:Oe,todayOrders:Ie,orderLoadMoreBtn:qe,orderSearch:Ke,orderDateFilter:Qe,orderStatusFilter:_e,orderFilterResultCount:Re,message:Ue,orderFormTitle:He,submitOrderBtn:je,cartTabBadge:Fe,syncNavBadge:Ve,networkStatus:Ge,mobileGlobalStatus:ze,debtList:Xe,debtLoadMoreBtn:Ye,debtLedgerList:We,debtTotalAmount:Je,debtCustomerCount:Ze,debtPendingAmount:et,debtTabMessage:tt,debtCustomersSubtab:nt,debtCollectSubtab:rt,debtCustomersPanel:ot,debtCollectPanel:at,debtCustomerSearch:st,debtCustomerSort:it}=S(),ct=window.MobileUiRuntime,dt=ct.createLifecycle(),lt=ct.createChunkedHtmlRenderer(pe,{
initialCount:60,chunkSize:80}),ut=ct.createChunkedHtmlRenderer(Xe,{initialCount:60,chunkSize:80}),mt=ct.createChunkedHtmlRenderer(Ie,{initialCount:60,chunkSize:80}),ht=C({tabs:me,
...
orderTab:"#ban-hang",cartTab:"#gio-hang",debtTab:"#cong-no",reportTab:"#don-hang"},onActivate(e){ue.ui.activeTabId=e,"debtTab"===e&&_t(),"reportTab"===e&&zt(),
"orderTab"!==e&&"cartTab"!==e||Kt()}});function gt(e,t={}){ue.ui.activeTabId=ht.switchPanel(e,t)}const pt=k(ze);function bt(){if(!Ge)return;const e=!1!==navigator.onLine
...
className:t.baseClass||"order-list"})}function yt(){return re(_e)}function vt(e={}){return te(e,yt())}function wt(e={}){return ne(e,Ke?.value||"")}function Ct(){
return ue.draft.isDirty(Pe?.value||0)}function kt(){ue.draft.persist(Pe?.value||"")}function St(){
if(!ue.draft.customer)return we.textContent="Chưa chọn khách hàng. Hãy sang tab Khách hàng để chọn.",we.classList.add("muted"),
void(Ne&&(Ne.textContent="Chưa chọn khách hàng cho đơn này.",Ne.classList.add("muted")));const e=ue.draft.customer,t=A(e),n=$(e),r=L(e),o={
heading:`${t||""}${t&&n?" · ":""}${n||""}`,lines:[`${/^chưa có/i.test(r)?"":`${r} · `}Nợ ${g(x(e))} · DS tháng ${g(B(e))}`]};window.SafeDom.renderSummary(we,o),
we.classList.remove("muted"),Ne&&(window.SafeDom.renderSummary(Ne,{...o,prefix:"Đơn đang lập cho"}),Ne.classList.remove("muted"))}function Pt(e={}){return de(e,{customerName:$,
customerCode:A})}async function Et(){try{const e=await c({statuses:["pending","failed","conflict","needs_attention"],limit:100})
;ue.sync.pendingOrders=e.filter(e=>"sales_order_create"===e.type).map(Pt),Ve&&(Ve.textContent=String(ue.sync.pendingOrders.length),Ve.hidden=0===ue.sync.pendingOrders.length),
Gt(ue.orders.rows)}catch(e){ue.sync.pendingOrders=[],Ve&&(Ve.hidden=!0)}}async function Nt(e="",t={}){const n=!0===t.append;if(ue.customer.loading)return
...
const t=N(ue.debt.rows),n=(Array.isArray(e)?e:[]).map(e=>I(e,t)).sort((e,t)=>x(t)-x(e));ue.customer.rows=n,n.length?(pe.className="customer-list",lt.render(n,(e,t)=>{
const n=A(e),r=$(e),o=x(e),a=M(e),s=L(e)
...
priceLocked:!0}})}}function $t(e={}){return z(e,{formatStock:Tt})}function Dt(){return V(ye?.value||"")}function Mt(){ue.draft.product=null,fe&&(fe.dataset.id="",
fe.dataset.code="",fe.dataset.name="",fe.dataset.type=""),Ce.textContent="",Ce.hidden=!0,Ce.classList.add("muted"),Ce.classList.remove("mobile-selected-product-compact-meta")}
function Bt(e={}){const t=e.internalSaleQuota?.snapshotDate||e.internalSaleQuota?.date||e.internalSaleQuota?.importDate||"";return t?m(t):"chưa cập nhật"}function Ot(e={}){
const t=function(e={}){return Number(e.finalPrice||e.unitPrice||e.salePrice||e.price||0)}(e),n=function(e={}){
return Number(e.originalPrice||e.grossPrice||e.catalogSalePrice||e.salePrice||e.price||0)}(e),r=n>t&&t>0?`Giá KM ${g(t)} · Gốc ${g(n)}`:`Giá ${g(t)}`
;return[`Tồn ${e.stockDisplay||Tt(e.availableQty,e.conversionRate)}`,`App ${Tt(e.maxOrderQty,e.conversionRate)}`,r,`DMS ${Bt(e)}`].join(" · ")}function It(){if(!fe||!ve)return
;const e=fe.closest(".clearable-search-control"),t=document.querySelector("#orderTab .mobile-product-search-first-card")
;e&&t&&ve.parentElement===e&&e.insertAdjacentElement("afterend",ve),t&&ve.parentElement!==t&&e?.parentElement===t&&e.insertAdjacentElement("afterend",ve)}function qt(e){
const t=$t(e);ue.draft.product=t,fe.dataset.id=t.id||"",fe.dataset.code=t.code||"",fe.dataset.name=t.name||"",fe.dataset.type="product",
fe.value=[t.code,t.name].filter(Boolean).join(" · "),function(e={}){Ce&&(Ce.textContent=Ot(e),Ce.hidden=!1,Ce.classList.remove("muted"),
Ce.classList.add("mobile-selected-product-compact-meta"))}(t),ve.innerHTML="",ve.classList.remove("has-many"),ve.hidden=!0,ve.style.display="none",It(),Se.focus()}function Kt(){
ue.product.toolsInitialized||(ue.product.toolsInitialized=!0,fe&&ve&&(window.SearchAutocomplete&&window.UnifiedProductSearch?(function(){
if(!window.UnifiedProductSearch||!0===window.UnifiedProductSearch.__mobileSalesCompactLabel)return;const e=window.UnifiedProductSearch.labelHtml
;window.UnifiedProductSearch.labelHtml=(t={},n="sales")=>{if("sales"!==n)return"function"==typeof e?e(t,n):u(String(t.label||""))
;const r=$t(t),o=[r.code,r.name].filter(Boolean).join(" · ")
;return`<div class="mobile-product-suggest-title">${u(o)}</div><div class="mobile-product-suggest-meta">${u(Ot(r))}</div>`},window.UnifiedProductSearch.__mobileSalesCompactLabel=!0
}(),window.SearchAutocomplete.wire({input:fe,box:ve,getItems:()=>async function(e=""){const t=String(e||"").trim();if(t.length<2)return[];try{const e=await r.getProducts(t,{
limit:20,group:Dt(),inStockOnly:"1"}),n=G(e).map($t).filter(e=>Number(e.availableQty||e.availableStock||e.stockQuantity||0)>0)
...
const e=await window.UnifiedSearchEngine.searchProduct(t,{limit:20,mode:"sales",includeStock:1,inStockOnly:1,group:Dt()})
;return G(e).map($t).filter(e=>Number(e.availableQty||e.availableStock||e.stockQuantity||0)>0)}
if(window.UnifiedProductSearch&&"function"==typeof window.UnifiedProductSearch.search){const e=await window.UnifiedProductSearch.search(t,{limit:20,mode:"sales",group:Dt()})
;return G(e).map($t).filter(e=>Number(e.availableQty||e.availableStock||e.stockQuantity||0)>0)}return[]}(fe.value.trim()),
...
select:qt,emptyText:"Không tìm thấy sản phẩm phù hợp"}),It(),fe.addEventListener("input",()=>{Mt(),It()}),ye?.addEventListener("change",()=>{Mt(),fe&&(fe.value=""),
ve&&(ve.innerHTML="",ve.classList.remove("has-many"),ve.hidden=!0,ve.style.display="none")}),async function(e=!1){if(ye&&(!ue.product.groupOptionsLoaded||e)){
ue.product.groupOptionsLoaded=!0;try{const e=await r.getProductGroups();!function(e=[]){if(!ye)return
;const t=Dt(),n=[...new Set((e||[]).map(V).filter(Boolean))].sort((e,t)=>e.localeCompare(t,"vi",{numeric:!0}));ye.replaceChildren();const r=document.createElement("option")
;r.value="",r.textContent="Tất cả nhóm hàng",ye.appendChild(r),n.forEach(e=>{const t=document.createElement("option");t.value=e,t.textContent=e,ye.appendChild(t)}),
t&&n.includes(t)&&(ye.value=t)}(e.items||e.groups||[])}catch(e){ue.product.groupOptionsLoaded=!1,
"REQUEST_ABORTED"!==e?.code&&console.warn("[mobile-sales] không tải được nhóm hàng sản phẩm:",e.message||e)}}}(),fe.addEventListener("focus",()=>{It(),
...
})):ve.innerHTML='<div class="suggestion-empty">Thiếu engine gợi ý sản phẩm dùng chung.</div>'))}function Qt(e,t={}){const n="collect"===e?"collect":"customers";ue.debt.subtab=n,
...
})):!1!==t.scroll&&document.getElementById("debtTab")?.scrollIntoView({block:"start",behavior:t.behavior||"smooth"})}async function _t(e={}){
...
;if(ue.debt.loaded&&!n&&!t)return void Rt(ue.debt.rows,ue.debt.summary);const a=++ue.debt.requestSeq,s=t?ue.debt.page+1:1;ue.debt.loading=!0,f(Ye,!0,"Đang tải...");try{
...
Rt(ue.debt.rows,ue.debt.summary),Ye&&(Ye.hidden=!ue.debt.hasMore),Array.isArray(ue.customer.rows)&&ue.customer.rows.length&&Lt(ue.customer.rows)}catch(e){
...
a===ue.debt.requestSeq&&(ue.debt.loading=!1),f(Ye,!1)}}function Rt(e=ue.debt.rows,t={}){
const n=Number(t.totalDebt??e.reduce((e,t)=>e+Number(t.debtAmount||0),0)),r=Number(t.pendingCollected??e.reduce((e,t)=>e+D(t),0));if(Je&&(Je.textContent=g(n)),
Ze&&(Ze.textContent=String(t.customerCount??e.length)),et&&(et.textContent=g(r)),Ht(e),ue.debt.selectedCustomerKey){
const e=ue.debt.selectedCustomerKey&&ue.debt.rows.find(e=>ae(e)===ue.debt.selectedCustomerKey)||null;e?ue.debt.formDirty||Ft(e):(ue.debt.selectedCustomerKey="",
ue.debt.formDirty=!1,Ft())}else Ft()}function Ut(){St(),kt(),function(){const e=Y(ue.draft.cart);Le.textContent=`${ue.draft.cart.length} dòng`,
...
De&&(De.textContent=`${ue.draft.cart.length} sản phẩm`),Me&&(Me.textContent=g(e.payable)),$e&&($e.hidden=0===ue.draft.cart.length),
je&&(je.disabled=!ue.draft.customer||0===ue.draft.cart.length)}(),ue.draft.cart.length?(Ee.className="cart-list",Ee.innerHTML=v(ue.draft.cart,{escapeHtml:u,money:g,
normalizePackingRate:F,quantityDisplay:At})):ft(Ee,{state:"empty",baseClass:"cart-list",title:"Giỏ hàng chưa có sản phẩm",
detail:ue.draft.customer?"Quay lại Bán hàng để chọn sản phẩm.":"Hãy chọn khách hàng trước khi lập đơn."})}function Ht(e=ue.debt.rows){if(!Xe)return;const t=Array.isArray(e)?e:[]
...
availableDebt:Number(e.debt||0),pendingCollectedAmount:0}))}function Ft(e={}){if(!We)return;if(!ae(e))return We.className="order-list empty",
...
void document.getElementById("chooseDebtCustomerBtn")?.addEventListener("click",()=>Qt("customers"));const t=Array.isArray(e.ledgers)?e.ledgers:[],n=jt(e);let o=0
...
const t=jt(e),n=[...document.querySelectorAll(".mobile-debt-order-check:checked")].reduce((e,n)=>{const r=t[Number(n.dataset.index)]
...
const n=jt(e),r=[...document.querySelectorAll(".mobile-debt-order-check:checked")].map(e=>Number(e.dataset.index)).filter(e=>Number.isFinite(e));let o=Math.max(0,Number(t||0))
...
ue.debt.selectedCustomerKey="",ue.debt.loaded=!1,await _t({force:!0}),Qt("customers",{restoreScroll:!0})}catch(e){
...
}finally{f(l,!1)}}(t,e)))}function Vt(e=!0){ue.draft.cart=[],ue.draft.editingOrderId="",ue.draft.product=null,fe.value="",ke.value="",Se.value="",Pe.value="",Mt(),
He.textContent="Đặt hàng",je.textContent="Xác nhận đơn",e&&(ue.draft.customer=null,y(Ue,"Đã làm mới đơn. Hãy chọn khách hàng ở tab 1.","success")),St(),Ut(),
Ct()||ue.draft.clearPersistence()}function Gt(e=ue.orders.rows,n=ue.orders.summary){ue.orders.rows=Array.isArray(e)?e:[]
;const r=ue.orders.rows,o=String(Qe?.value||t()),a=[...ue.sync.pendingOrders.filter(e=>!o||String(e.date||"").slice(0,10)===o),...r],s=a.filter(wt).filter(vt),i=Number(n?.totalAmount??r.reduce((e,t)=>e+Number(t.totalAmount||0),0)),c=Number(n?.paidAmount??r.reduce((e,t)=>e+Number(t.paidAmount||0),0)),d=Number(n?.debtAmount??r.reduce((e,t)=>e+Number(t.debtAmount||0),0)),l=Number(n?.orderCount??r.length)
...
document.getElementById("todayDebt").textContent=g(d),Re&&(Re.textContent=`${s.length} đơn`),qe&&(qe.hidden=!ue.orders.hasMore||"pending_sync"===yt()),!s.length){const e=a.length>0
;return void ft(Ie,{state:"empty",baseClass:"order-list",title:e?"Không có đơn phù hợp bộ lọc":"Chưa có đơn trong ngày đã chọn",
detail:e?"Hãy đổi từ khóa hoặc trạng thái hiển thị.":"Đơn online và đơn chờ đồng bộ sẽ xuất hiện tại đây."})}Ie.className="order-list mobile-order-list",mt.render(s,e=>w([e],{
escapeHtml:u,money:g,formatDate:m}))}async function zt(e={}){const n=!0===e.append,o=!0===e.force,a=Z({date:Qe?.value||t(),q:Ke?.value||""})
;if(n&&ue.orders.loadedKey!==a)return zt({reset:!0,force:!0});if(ue.orders.loading)return;if(n&&!ue.orders.hasMore)return
;if(ue.orders.loaded&&ue.orders.loadedKey===a&&!o&&!n)return void Gt(ue.orders.rows,ue.orders.summary);const s=++ue.orders.requestSeq,i=n?ue.orders.page+1:1;ue.orders.loading=!0,
...
;const o=e.items||[],c=function(e=[]){return R(e,le)}(o);ue.orders.page=Number(e.pagination?.page||i),ue.orders.hasMore=Boolean(e.pagination?.hasMore),
ue.orders.summary=e.summary||ue.orders.summary||{},ue.orders.rows=n?ee(ue.orders.rows,c):c,ue.orders.loaded=!0,ue.orders.loadedKey=a,Gt(ue.orders.rows,ue.orders.summary),
qe&&(qe.hidden=!ue.orders.hasMore||"pending_sync"===yt()),o.length!==c.length&&console.warn("[MOBILE_SALES_OWNER_GUARD]",{currentSalesStaffCode:_(le),received:o.length,
...
me.forEach(e=>e.addEventListener("click",()=>gt(e.dataset.tab))),Be?.addEventListener("click",()=>gt("cartTab")),Oe?.addEventListener("click",()=>gt("orderTab",{
...
const t=K(I(e,N(ue.debt.rows))),n=Q(ue.draft.customer||{})[0]||"",r=Q(t)[0]||"";if(Boolean(ue.draft.customer&&(n||r)&&n!==r)&&(ue.draft.cart.length||ue.draft.editingOrderId)){
...
Pe.value="",Ut()}ue.draft.customer=t,St(),kt(),y(Ue,"Đã chọn khách hàng. Hãy thêm sản phẩm vào giỏ.","success"),gt("orderTab"),Kt(),setTimeout(()=>fe.focus(),200)}(n)}),
...
ue.debt.selectedCustomerKey=t,ue.debt.formDirty=!1,Ft(e),Qt("collect")):Qt("collect"))}(n)}),dt.listen(window,"pagehide",()=>{lt.cancel(),ut.cancel(),mt.cancel(),dt.destroy()},{
once:!0}),document.addEventListener("click",e=>{const t=e.target.closest("[data-mobile-retry]");if(!t)return;const n=t.dataset.mobileRetry;"customers"===n&&Nt(ge.value.trim(),{
reset:!0,force:!0}),"orders"===n&&zt({reset:!0,force:!0}),"debts"===n&&_t({reset:!0,force:!0})}),ge.addEventListener("input",l(()=>Nt(ge.value.trim(),{reset:!0}),250)),
document.getElementById("reloadCustomersBtn")?.addEventListener("click",()=>{window.CatalogCache&&window.CatalogCache.invalidate("customers"),Nt(ge.value.trim(),{reset:!0,force:!0
})}),be?.addEventListener("click",()=>Nt(ue.customer.query,{append:!0})),document.getElementById("reloadOrdersBtn")?.addEventListener("click",()=>zt({reset:!0,force:!0})),
qe?.addEventListener("click",()=>zt({append:!0})),Ye?.addEventListener("click",()=>_t({append:!0})),Ke?.addEventListener("input",l(()=>{ue.orders.loaded=!1,ue.orders.loadedKey="",
zt({reset:!0,force:!0})},300)),Qe?.addEventListener("change",()=>{ue.orders.loaded=!1,ue.orders.loadedKey="",zt({reset:!0,force:!0})}),
_e?.addEventListener("change",()=>Gt(ue.orders.rows,ue.orders.summary)),Ie?.addEventListener("click",async e=>{const t=e.target.closest("[data-edit-order]");if(t&&Ie.contains(t)){
...
Pe.value=Number(t.paidAmount||0),He.textContent=`Sửa đơn ${t.code||""}`,je.textContent=`Lưu sửa đơn ${t.code||""}`,await xt({silent:!0}),Ut()
...
;ue.orders.rows=ue.orders.rows.filter(n=>String(n.id||n.code||"")!==String(e||"")&&String(n.code||"")!==String(t||"")),Gt(ue.orders.rows,ue.orders.summary),await zt({reset:!0,
...
ue.debt.formDirty&&!window.confirm("Bạn đang có phiếu thu chưa gửi. Tải lại sẽ xóa dữ liệu đang nhập.")||(ue.debt.formDirty=!1,_t({reset:!0,force:!0}))}),
nt?.addEventListener("click",()=>Qt("customers")),rt?.addEventListener("click",()=>Qt("collect")),st?.addEventListener("input",()=>Ht(ue.debt.rows)),
it?.addEventListener("change",()=>Ht(ue.debt.rows)),document.getElementById("clearOrderBtn")?.addEventListener("click",()=>{
Ct()&&!window.confirm("Làm mới sẽ xóa khách hàng và toàn bộ giỏ đang nhập. Bạn có chắc không?")||Vt(!0)}),document.getElementById("logoutBtn")?.addEventListener("click",e=>{
Ct()&&(window.confirm("Bạn đang có đơn chưa lưu. Thoát ứng dụng vẫn giữ bản nháp trên thiết bị. Bạn có chắc muốn thoát?")||(e.preventDefault(),e.stopImmediatePropagation()))},!0),
Pe?.addEventListener("input",kt),window.addEventListener("beforeunload",e=>{Ct()&&(e.preventDefault(),e.returnValue="")}),window.addEventListener("mkpro:offline-queued",e=>{
"sales_order_create"===e.detail?.type&&(Et(),pt("Đơn đã được lưu trên thiết bị và đang chờ đồng bộ.","warning",{persist:!0}))}),
window.addEventListener("mkpro:offline-synced",async()=>{await Et(),ue.orders.loaded&&await zt({reset:!0,force:!0}),pt("Đã đồng bộ dữ liệu chờ lên máy chủ.","success")}),
...
Qe&&!Qe.value&&(Qe.value=t()),Ee&&"1"!==Ee.dataset.phase3Bound&&(Ee.dataset.phase3Bound="1",Ee.addEventListener("click",async e=>{const t=e.target.closest("[data-remove]")
;if(t&&Ee.contains(t)){const e=Number(t.dataset.remove),n=ue.draft.cart[e];if(!n)return;if(!window.confirm(`Xóa ${n.productName||n.productCode} khỏi giỏ hàng?`))return
;return ue.draft.cart.splice(e,1),await xt({silent:!0}),Ut(),void pt("Đã xóa sản phẩm khỏi giỏ hàng.","success")}const n=e.target.closest("[data-cart-update]")
;n&&Ee.contains(n)&&await async function(e,t){const n=ue.draft.cart[e];if(!n)return
;const r=Ee.querySelector(`[data-cart-case="${e}"]`),o=Ee.querySelector(`[data-cart-loose="${e}"]`),{rate:a,quantity:s}=W(n,r?.value,o?.value),i=J(n,s)
;if(i.ok||"INVALID_QUANTITY"!==i.code)if(i.ok||"OVER_STOCK"!==i.code)if(i.ok||"OVER_APP_QUOTA"!==i.code){f(t,!0,"Đang tính...");try{n.quantity=s,await xt({silent:!0}),Ut(),
...
}(Number(n.dataset.cartUpdate),n)})),Ft(),Qt("customers",{restoreScroll:!1}),function(){const e=ue.draft.restore();return!!e&&(e.customer&&(ue.draft.customer=K(e.customer)),
Pe&&(Pe.value=e.paidAmount),St(),ue.draft.editingOrderId&&(He.textContent=`Tiếp tục sửa đơn ${ue.draft.editingOrderId}`,je.textContent=`Lưu sửa đơn ${ue.draft.editingOrderId}`),
Ct())}()&&pt("Đã khôi phục đơn đang nhập trên thiết bị này.","success");const e=r.getRuntimeConfig().catch(()=>null);await Et(),await Nt("",{reset:!0}),await e,Ut(),
...
;const e=Number(ke?.value||0),t=Number(Se?.value||0),n=F(ue.draft.product),r=(e>0&&n>0?e*n:0)+t;if(r<=0)return y(Ue,"Số lượng phải lớn hơn 0","error")
...
;ue.draft.product=null,fe.value="",ke.value="",Se.value="",Ce.textContent="",Ce.hidden=!0,Ce.classList.add("muted"),await xt(),Ut(),
y(Ue,"Đã thêm vào giỏ hàng và áp giá sau khuyến mại","success"),ve&&(ve.innerHTML="",ve.classList.remove("has-many"),ve.hidden=!0,ve.style.display="none"),
window.requestAnimationFrame(()=>fe?.focus())}),je.addEventListener("click",async()=>{if(je.disabled)return;if(y(Ue,""),
...
;if(!ue.draft.cart.length)return y(Ue,"Chưa có sản phẩm","error");f(je,!0);let t=null;try{const n=Number(Pe.value||0);await xt({silent:!0});const o={customer:e,
...
;window.CatalogCache&&window.CatalogCache.invalidate("products"),Vt(!1),function(e={}){ue.orders.rows=oe(ue.orders.rows,e),Gt(ue.orders.rows,ue.orders.summary)}(a.salesOrder)
;const i=`${a.message||"Đã lưu đơn"} ${s}`.trim();y(Ue,i,"success"),pt(i,"success"),ue.debt.loaded&&await _t({reset:!0,force:!0}),await zt({reset:!0,force:!0}),gt("reportTab")
}catch(e){if(!ue.draft.editingOrderId&&t&&i(e)&&s("sales_order_create")){await a("sales_order_create",t),Vt(!1)
;const e="Đã lưu đơn offline. Đơn đang hiển thị trong danh sách Chờ đồng bộ.";y(Ue,e,"success"),pt(e,"warning",{persist:!0}),await Et(),gt("reportTab")}else if(i(e)){kt()
