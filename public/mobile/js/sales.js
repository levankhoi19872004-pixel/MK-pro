...
;import{createMobileSalesState as L,OrderDraftStore as T}from"./sales/state.js?v=phase86-production-hardening-v1"
;import{buildDebtLookup as N,customerAddressValue as P,customerAvailableDebtValue as E,customerCodeValue as $,customerDebtValue as A,customerNameValue as x,customerPendingCollectedValue as D,customerPhoneValue as M,customerSalesValue as B,debtClassName as I,mergeCustomerDebt as O,mergeCustomerPages as q,normalizeSelectedCustomerForSubmit as K,uniqueCustomerIdentityKeys as Q}from"./sales/customer.js?v=phase86-production-hardening-v1"
...
;import{applyPromotionLines as H,attachPackingRate as U,buildPromotionCartPayloadItem as j,normalizePackingRate as V,normalizeProductGroupName as F,normalizeProductSearchResponse as G,toMobileProduct as z}from"./sales/product.js?v=phase86-production-hardening-v1"
...
;document.getElementById("staffInfo").textContent=`${le.name||le.username||"Nhân viên"} · ${le.role||"sales"}`;const ue=L({draftStore:new T({
ownerKey:R(le)||le.id||le.username||"sales"})
}),{tabs:me,panels:he,customerSearch:ge,customerList:pe,customerLoadMoreBtn:be,productSearch:fe,productGroupFilter:ye,productSuggestions:ve,selectedCustomerBox:Ce,selectedProductBox:we,caseQtyInput:ke,looseQtyInput:Se,paidAmountInput:Le,cartList:Te,cartCustomerContext:Ne,cartCount:Pe,cartTotal:Ee,cartGrossTotal:$e,cartDiscountTotal:Ae,orderDraftBar:xe,orderDraftLineCount:De,orderDraftTotal:Me,openCartBtn:Be,backToOrderBtn:Ie,todayOrders:Oe,orderLoadMoreBtn:qe,orderSearch:Ke,orderDateFilter:Qe,orderStatusFilter:Re,orderFilterResultCount:_e,message:He,orderFormTitle:Ue,submitOrderBtn:je,cartTabBadge:Ve,syncNavBadge:Fe,networkStatus:Ge,mobileGlobalStatus:ze,debtList:Xe,debtLoadMoreBtn:Ye,debtLedgerList:We,debtTotalAmount:Je,debtCustomerCount:Ze,debtPendingAmount:et,debtTabMessage:tt,debtCustomersSubtab:nt,debtCollectSubtab:rt,debtCustomersPanel:ot,debtCollectPanel:at,debtCustomerSearch:st,debtCustomerSort:it}=S(),ct=window.MobileUiRuntime,dt=ct.createLifecycle(),lt=ct.createChunkedHtmlRenderer(pe,{
...
orderTab:"#ban-hang",cartTab:"#gio-hang",debtTab:"#cong-no",reportTab:"#don-hang"},onActivate(e){ue.ui.activeTabId=e,"debtTab"===e&&Kt(),"reportTab"===e&&Ft(),
"orderTab"!==e&&"cartTab"!==e||Ot()}});function gt(e,t={}){ue.ui.activeTabId=ht.switchPanel(e,t)}const pt=k(ze);function bt(){if(!Ge)return;const e=!1!==navigator.onLine
...
return ue.draft.isDirty(Le?.value||0)}function kt(){ue.draft.persist(Le?.value||"")}function St(){
...
;const e=$(ue.draft.customer),t=x(ue.draft.customer),n=`<strong>${u(e||"")}${e&&t?" - ":""}${u(t||"")}</strong><br /><span>SĐT: ${u(M(ue.draft.customer))}</span><br /><span>ĐC: ${u(P(ue.draft.customer))}</span><br /><span>Nợ: ${g(A(ue.draft.customer))} · DS tháng: ${g(B(ue.draft.customer))}</span>`
;Ce.innerHTML=n,Ce.classList.remove("muted"),Ne&&(Ne.innerHTML=`<span>Đơn đang lập cho</span><br />${n}`,Ne.classList.remove("muted"))}function Lt(e={}){return de(e,{
customerName:x,customerCode:$})}async function Tt(){try{const e=await c({statuses:["pending","failed","conflict","needs_attention"],limit:100})
;ue.sync.pendingOrders=e.filter(e=>"sales_order_create"===e.type).map(Lt),Fe&&(Fe.textContent=String(ue.sync.pendingOrders.length),Fe.hidden=0===ue.sync.pendingOrders.length),
Vt(ue.orders.rows)}catch(e){ue.sync.pendingOrders=[],Fe&&(Fe.hidden=!0)}}async function Nt(e="",t={}){const n=!0===t.append;if(ue.customer.loading)return
...
Pt(ue.customer.rows),be&&(be.hidden=!ue.customer.hasMore)}catch(e){if(o!==ue.customer.requestSeq||"REQUEST_ABORTED"===e?.code)return
...
detail:e.message||"Vui lòng kiểm tra kết nối và thử lại.",retryAction:"customers"})}finally{o===ue.customer.requestSeq&&(ue.customer.loading=!1),f(be,!1)}}function Pt(e){
const t=N(ue.debt.rows),n=(Array.isArray(e)?e:[]).map(e=>O(e,t)).sort((e,t)=>A(t)-A(e));ue.customer.rows=n,n.length?(pe.className="customer-list",lt.render(n,(e,t)=>{
const n=$(e),r=x(e),o=A(e),a=M(e),s=P(e)
...
detail:ge.value.trim()?"Hãy thử từ khóa ngắn hơn hoặc kiểm tra mã khách.":"Danh sách khách hàng phụ trách đang trống."})}function Et(e,t){return n(e,t).display}function $t(e={}){
const t=V(e);return Et(Number(e.quantity||e.qty||0),t)}async function At(e={}){if(!ue.draft.cart.length)return;const n=!!e.silent;try{const e=await r.calculatePromotions({date:t(),
...
priceLocked:!0}})}}function xt(e={}){return z(e,{formatStock:Et})}function Dt(e=""){
return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Mt(){return F(ye?.value||"")}
function Bt(){ue.draft.product=null,fe&&(fe.dataset.id="",fe.dataset.code="",fe.dataset.name="",fe.dataset.type=""),we.textContent="Chưa chọn sản phẩm",we.classList.add("muted")}
function It(e){const t=xt(e);ue.draft.product=t,fe.dataset.id=t.id||"",fe.dataset.code=t.code||"",fe.dataset.name=t.name||"",fe.dataset.type="product",
fe.value=t.label||[t.code,t.name].filter(Boolean).join(" - ")
...
;we.innerHTML=`\n    <div class="mobile-selected-product-name">${u(t.code||"")} - ${u(t.name||"")}</div>\n    <div class="mobile-selected-product-meta">\n      <span>Tồn thực tế<strong>${u(t.stockDisplay||Et(t.availableQty,t.conversionRate))}</strong></span>\n      <span class="mobile-app-quota-meta">Được bán App<strong>${u(Et(t.maxOrderQty,t.conversionRate))}</strong></span>\n      <span>${o}</span>\n      ${a}\n    </div>\n    <div class="mobile-selected-product-quota-note">Hạn mức theo file DMS: ${u(t.internalSaleQuota?.snapshotDate||"chưa cập nhật")}</div>\n  `,
we.classList.remove("muted"),ve.innerHTML="",ve.classList.remove("has-many"),ve.hidden=!0,ve.style.display="none",Se.focus()}function Ot(){
ue.product.toolsInitialized||(ue.product.toolsInitialized=!0,fe&&ve&&(window.SearchAutocomplete&&window.UnifiedProductSearch?(window.SearchAutocomplete.wire({input:fe,box:ve,
getItems:()=>async function(e=""){const t=String(e||"").trim();if(t.length<2)return[];try{const e=await r.getProducts(t,{limit:50,group:Mt()}),n=G(e).map(xt)
...
const e=await window.UnifiedSearchEngine.searchProduct(t,{limit:50,mode:"sales",includeStock:1,group:Mt()});return G(e).map(xt)}
if(window.UnifiedProductSearch&&"function"==typeof window.UnifiedProductSearch.search){const e=await window.UnifiedProductSearch.search(t,{limit:50,mode:"sales",group:Mt()})
;return G(e).map(xt)}return[]}(fe.value.trim()),
...
select:It,emptyText:"Không tìm thấy sản phẩm phù hợp"}),fe.addEventListener("input",Bt),ye?.addEventListener("change",()=>{Bt(),fe&&(fe.value=""),ve&&(ve.innerHTML="",
...
const e=await r.getProductGroups();!function(e=[]){if(!ye)return;const t=Mt(),n=[...new Set((e||[]).map(F).filter(Boolean))].sort((e,t)=>e.localeCompare(t,"vi",{numeric:!0}))
;ye.innerHTML=['<option value="">Tất cả nhóm hàng</option>',...n.map(e=>`<option value="${Dt(e)}">${Dt(e)}</option>`)].join(""),t&&n.includes(t)&&(ye.value=t)
}(e.items||e.groups||[])}catch(e){ue.product.groupOptionsLoaded=!1,"REQUEST_ABORTED"!==e?.code&&console.warn("[mobile-sales] không tải được nhóm hàng sản phẩm:",e.message||e)}}}(),
fe.addEventListener("focus",()=>{fe.dispatchEvent(new Event("input",{bubbles:!0}))}),fe.addEventListener("keydown",e=>{"Escape"===e.key&&(ve.innerHTML="",
ve.classList.remove("has-many"))})):ve.innerHTML='<div class="suggestion-empty">Thiếu engine gợi ý sản phẩm dùng chung.</div>'))}function qt(e,t={}){
const n="collect"===e?"collect":"customers";ue.debt.subtab=n,nt?.classList.toggle("active","customers"===n),rt?.classList.toggle("active","collect"===n),
nt?.setAttribute("aria-selected",String("customers"===n)),rt?.setAttribute("aria-selected",String("collect"===n)),ot?.classList.toggle("active","customers"===n),
at?.classList.toggle("active","collect"===n),"collect"!==n?!1!==t.restoreScroll&&window.requestAnimationFrame(()=>window.scrollTo({top:ue.debt.listScrollTop,behavior:"auto"
})):!1!==t.scroll&&document.getElementById("debtTab")?.scrollIntoView({block:"start",behavior:t.behavior||"smooth"})}async function Kt(e={}){
...
;if(ue.debt.loaded&&!n&&!t)return void Qt(ue.debt.rows,ue.debt.summary);const a=++ue.debt.requestSeq,s=t?ue.debt.page+1:1;ue.debt.loading=!0,f(Ye,!0,"Đang tải...");try{
...
Qt(ue.debt.rows,ue.debt.summary),Ye&&(Ye.hidden=!ue.debt.hasMore),Array.isArray(ue.customer.rows)&&ue.customer.rows.length&&Pt(ue.customer.rows)}catch(e){
...
a===ue.debt.requestSeq&&(ue.debt.loading=!1),f(Ye,!1)}}function Qt(e=ue.debt.rows,t={}){
...
Ze&&(Ze.textContent=String(t.customerCount??e.length)),et&&(et.textContent=g(r)),_t(e),ue.debt.selectedCustomerKey){
...
ue.debt.formDirty=!1,Ut())}else Ut()}function Rt(){St(),kt(),function(){const e=Y(ue.draft.cart);Pe.textContent=`${ue.draft.cart.length} dòng`,
Ve&&(Ve.textContent=String(ue.draft.cart.length)),$e&&($e.textContent=g(e.gross)),Ae&&(Ae.textContent=e.discount>0?`-${g(e.discount)}`:g(0)),Ee.textContent=g(e.payable),
De&&(De.textContent=`${ue.draft.cart.length} sản phẩm`),Me&&(Me.textContent=g(e.payable)),xe&&(xe.hidden=0===ue.draft.cart.length),
je&&(je.disabled=!ue.draft.customer||0===ue.draft.cart.length)}(),ue.draft.cart.length?(Te.className="cart-list",Te.innerHTML=v(ue.draft.cart,{escapeHtml:u,money:g,
normalizePackingRate:V,quantityDisplay:$t})):ft(Te,{state:"empty",baseClass:"cart-list",title:"Giỏ hàng chưa có sản phẩm",
detail:ue.draft.customer?"Quay lại Bán hàng để chọn sản phẩm.":"Hãy chọn khách hàng trước khi lập đơn."})}function _t(e=ue.debt.rows){if(!Xe)return;const t=Array.isArray(e)?e:[]
...
ut.render(n,({item:e,originalIndex:t})=>{const n=E(e),r=n<=0
...
})):ft(Xe,{state:"empty",baseClass:"order-list",title:"Không tìm thấy khách hàng phù hợp",detail:"Hãy thử mã khách, tên hoặc số điện thoại khác."})}function Ht(e={}){
...
void document.getElementById("chooseDebtCustomerBtn")?.addEventListener("click",()=>qt("customers"));const t=Array.isArray(e.ledgers)?e.ledgers:[],n=Ht(e);let o=0
...
const t=Ht(e),n=[...document.querySelectorAll(".mobile-debt-order-check:checked")].reduce((e,n)=>{const r=t[Number(n.dataset.index)]
...
const n=Ht(e),r=[...document.querySelectorAll(".mobile-debt-order-check:checked")].map(e=>Number(e.dataset.index)).filter(e=>Number.isFinite(e));let o=Math.max(0,Number(t||0))
...
ue.debt.selectedCustomerKey="",ue.debt.loaded=!1,await Kt({force:!0}),qt("customers",{restoreScroll:!0})}catch(e){
...
}finally{f(l,!1)}}(t,e)))}function jt(e=!0){ue.draft.cart=[],ue.draft.editingOrderId="",ue.draft.product=null,fe.value="",ke.value="",Se.value="",Le.value="",
we.textContent="Chưa chọn sản phẩm",we.classList.add("muted"),Ue.textContent="Đặt hàng",je.textContent="Xác nhận đơn",e&&(ue.draft.customer=null,
y(He,"Đã làm mới đơn. Hãy chọn khách hàng ở tab 1.","success")),St(),Rt(),wt()||ue.draft.clearPersistence()}function Vt(e=ue.orders.rows,n=ue.orders.summary){
...
escapeHtml:u,money:g,formatDate:m}))}async function Ft(e={}){const n=!0===e.append,o=!0===e.force,a=Z({date:Qe?.value||t(),q:Ke?.value||""})
;if(n&&ue.orders.loadedKey!==a)return Ft({reset:!0,force:!0});if(ue.orders.loading)return;if(n&&!ue.orders.hasMore)return
;if(ue.orders.loaded&&ue.orders.loadedKey===a&&!o&&!n)return void Vt(ue.orders.rows,ue.orders.summary);const s=++ue.orders.requestSeq,i=n?ue.orders.page+1:1;ue.orders.loading=!0,
...
ue.orders.summary=e.summary||ue.orders.summary||{},ue.orders.rows=n?ee(ue.orders.rows,c):c,ue.orders.loaded=!0,ue.orders.loadedKey=a,Vt(ue.orders.rows,ue.orders.summary),
...
const t=K(O(e,N(ue.debt.rows))),n=Q(ue.draft.customer||{})[0]||"",r=Q(t)[0]||"";if(Boolean(ue.draft.customer&&(n||r)&&n!==r)&&(ue.draft.cart.length||ue.draft.editingOrderId)){
...
Le.value="",Rt()}ue.draft.customer=t,St(),kt(),y(He,"Đã chọn khách hàng. Hãy thêm sản phẩm vào giỏ.","success"),gt("orderTab"),Ot(),setTimeout(()=>fe.focus(),200)}(n)}),
...
;!t||E(e)<=0||(ue.debt.selectedCustomerKey!==t?ue.debt.formDirty&&ue.debt.selectedCustomerKey&&ue.debt.selectedCustomerKey!==t&&!window.confirm("Bạn đang có phiếu thu chưa gửi. Dữ liệu hiện tại sẽ bị xóa khi chuyển khách hàng.")||(ue.debt.listScrollTop=window.scrollY||document.documentElement.scrollTop||0,
ue.debt.selectedCustomerKey=t,ue.debt.formDirty=!1,Ut(e),qt("collect")):qt("collect"))}(n)}),dt.listen(window,"pagehide",()=>{lt.cancel(),ut.cancel(),mt.cancel(),dt.destroy()},{
...
reset:!0,force:!0}),"orders"===n&&Ft({reset:!0,force:!0}),"debts"===n&&Kt({reset:!0,force:!0})}),ge.addEventListener("input",l(()=>Nt(ge.value.trim(),{reset:!0}),250)),
...
})}),be?.addEventListener("click",()=>Nt(ue.customer.query,{append:!0})),document.getElementById("reloadOrdersBtn")?.addEventListener("click",()=>Ft({reset:!0,force:!0})),
qe?.addEventListener("click",()=>Ft({append:!0})),Ye?.addEventListener("click",()=>Kt({append:!0})),Ke?.addEventListener("input",l(()=>{ue.orders.loaded=!1,ue.orders.loadedKey="",
Ft({reset:!0,force:!0})},300)),Qe?.addEventListener("change",()=>{ue.orders.loaded=!1,ue.orders.loadedKey="",Ft({reset:!0,force:!0})}),
Re?.addEventListener("change",()=>Vt(ue.orders.rows,ue.orders.summary)),Oe?.addEventListener("click",async e=>{const t=e.target.closest("[data-edit-order]");if(t&&Oe.contains(t)){
...
Le.value=Number(t.paidAmount||0),Ue.textContent=`Sửa đơn ${t.code||""}`,je.textContent=`Lưu sửa đơn ${t.code||""}`,await At({silent:!0}),Rt()
...
;ue.orders.rows=ue.orders.rows.filter(n=>String(n.id||n.code||"")!==String(e||"")&&String(n.code||"")!==String(t||"")),Vt(ue.orders.rows,ue.orders.summary),await Ft({reset:!0,
force:!0});const o=n.message||"Đã xóa đơn";y(He,o,"success"),pt(o,"success")}catch(e){y(He,e.message,"error"),pt(e.message||"Không xóa được đơn.","error",{persist:!0})}
}(n.dataset.deleteOrder,n.dataset.orderCode)}finally{f(n,!1)}}}),document.getElementById("reloadDebtsBtn")?.addEventListener("click",()=>{
ue.debt.formDirty&&!window.confirm("Bạn đang có phiếu thu chưa gửi. Tải lại sẽ xóa dữ liệu đang nhập.")||(ue.debt.formDirty=!1,Kt({reset:!0,force:!0}))}),
nt?.addEventListener("click",()=>qt("customers")),rt?.addEventListener("click",()=>qt("collect")),st?.addEventListener("input",()=>_t(ue.debt.rows)),
it?.addEventListener("change",()=>_t(ue.debt.rows)),document.getElementById("clearOrderBtn")?.addEventListener("click",()=>{
wt()&&!window.confirm("Làm mới sẽ xóa khách hàng và toàn bộ giỏ đang nhập. Bạn có chắc không?")||jt(!0)}),document.getElementById("logoutBtn")?.addEventListener("click",e=>{
...
Le?.addEventListener("input",kt),window.addEventListener("beforeunload",e=>{wt()&&(e.preventDefault(),e.returnValue="")}),window.addEventListener("mkpro:offline-queued",e=>{
"sales_order_create"===e.detail?.type&&(Tt(),pt("Đơn đã được lưu trên thiết bị và đang chờ đồng bộ.","warning",{persist:!0}))}),
window.addEventListener("mkpro:offline-synced",async()=>{await Tt(),ue.orders.loaded&&await Ft({reset:!0,force:!0}),pt("Đã đồng bộ dữ liệu chờ lên máy chủ.","success")}),
...
Qe&&!Qe.value&&(Qe.value=t()),Te&&"1"!==Te.dataset.phase3Bound&&(Te.dataset.phase3Bound="1",Te.addEventListener("click",async e=>{const t=e.target.closest("[data-remove]")
;if(t&&Te.contains(t)){const e=Number(t.dataset.remove),n=ue.draft.cart[e];if(!n)return;if(!window.confirm(`Xóa ${n.productName||n.productCode} khỏi giỏ hàng?`))return
;return ue.draft.cart.splice(e,1),await At({silent:!0}),Rt(),void pt("Đã xóa sản phẩm khỏi giỏ hàng.","success")}const n=e.target.closest("[data-cart-update]")
;n&&Te.contains(n)&&await async function(e,t){const n=ue.draft.cart[e];if(!n)return
;const r=Te.querySelector(`[data-cart-case="${e}"]`),o=Te.querySelector(`[data-cart-loose="${e}"]`),{rate:a,quantity:s}=W(n,r?.value,o?.value),i=J(n,s)
;if(i.ok||"INVALID_QUANTITY"!==i.code)if(i.ok||"OVER_STOCK"!==i.code)if(i.ok||"OVER_APP_QUOTA"!==i.code){f(t,!0,"Đang tính...");try{n.quantity=s,await At({silent:!0}),Rt(),
pt(`Đã cập nhật số lượng ${n.productName||n.productCode}.`,"success")}finally{f(t,!1)}
}else y(He,`Số lượng vượt hạn mức bán App (${Et(i.maxOrderQty,a)}).`,"error");else y(He,`Số lượng vượt tồn đang hiển thị (${Et(i.availableQty,a)}).`,"error");else y(He,"Số lượng sau khi sửa phải lớn hơn 0. Hãy dùng nút Xóa nếu không mua sản phẩm này.","error")
}(Number(n.dataset.cartUpdate),n)})),Ut(),qt("customers",{restoreScroll:!1}),function(){const e=ue.draft.restore();return!!e&&(e.customer&&(ue.draft.customer=K(e.customer)),
Le&&(Le.value=e.paidAmount),St(),ue.draft.editingOrderId&&(Ue.textContent=`Tiếp tục sửa đơn ${ue.draft.editingOrderId}`,je.textContent=`Lưu sửa đơn ${ue.draft.editingOrderId}`),
wt())}()&&pt("Đã khôi phục đơn đang nhập trên thiết bị này.","success");const e=r.getRuntimeConfig().catch(()=>null);await Tt(),await Nt("",{reset:!0}),await e,Rt(),
...
;const e=Number(ke?.value||0),t=Number(Se?.value||0),n=V(ue.draft.product),r=(e>0&&n>0?e*n:0)+t;if(r<=0)return y(He,"Số lượng phải lớn hơn 0","error")
;const o=Number(ue.draft.product.availableQty||0),a=Math.max(0,Number(ue.draft.product.maxOrderQty||0));if(o>0&&r>o)return y(He,"Số lượng vượt tồn thực tế","error")
;if(r>a)return y(He,a>0?`Sản phẩm chỉ còn được bán qua App ${Et(a,n)}`:"Sản phẩm chưa có hạn mức bán qua App. Vui lòng cập nhật file tồn DMS buổi sáng.","error")
...
;if(o>0&&e>o)return y(He,"Tổng số lượng vượt tồn thực tế","error");if(e>a)return y(He,`Tổng số lượng vượt hạn mức bán App. Còn tối đa ${Et(a,n)}`,"error");i.quantity=e,
...
;ue.draft.product=null,fe.value="",ke.value="",Se.value="",we.textContent="Chưa chọn sản phẩm",we.classList.add("muted"),await At(),Rt(),
y(He,"Đã thêm vào giỏ hàng và áp giá sau khuyến mại","success")}),je.addEventListener("click",async()=>{if(je.disabled)return;if(y(He,""),
...
;if(!ue.draft.cart.length)return y(He,"Chưa có sản phẩm","error");f(je,!0);let t=null;try{const n=Number(Le.value||0);await At({silent:!0});const o={customer:e,
...
;window.CatalogCache&&window.CatalogCache.invalidate("products"),jt(!1),function(e={}){ue.orders.rows=oe(ue.orders.rows,e),Vt(ue.orders.rows,ue.orders.summary)}(a.salesOrder)
;const i=`${a.message||"Đã lưu đơn"} ${s}`.trim();y(He,i,"success"),pt(i,"success"),ue.debt.loaded&&await Kt({reset:!0,force:!0}),await Ft({reset:!0,force:!0}),gt("reportTab")
}catch(e){if(!ue.draft.editingOrderId&&t&&i(e)&&s("sales_order_create")){await a("sales_order_create",t),jt(!1)
;const e="Đã lưu đơn offline. Đơn đang hiển thị trong danh sách Chờ đồng bộ.";y(He,e,"success"),pt(e,"warning",{persist:!0}),await Tt(),gt("reportTab")}else if(i(e)){kt()
