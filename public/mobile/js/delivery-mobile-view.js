...
;body.innerHTML=selectedOrderSummary(order)+'<section class="m-workflow-step phase23"><b>Bước 1 · Hàng giao kiêm nhập hàng trả</b><span>NVGH kiểm từng dòng hàng. Nếu khách trả một phần, nhập số lượng trả ngay trên dòng sản phẩm rồi bấm Xác nhận hàng & thu tiền.</span></section><section class="m-product-summary phase23"><div><span>Số dòng</span><b>'+esc(baseRows.length)+"</b></div><div><span>Tổng SL giao</span><b>"+money(totalQty)+"</b></div><div><span>Giá trị hàng</span><b>"+money(totalAmount)+'</b></div></section><form id="mProductReturnForm" class="m-product-return-form"><div class="m-return-scroll products-with-return-input">'+(baseRows.map(function(it,idx){
...
}).join("")||'<div class="m-empty">Đơn chưa có dòng hàng để đối chiếu.</div>')+'</div><div class="m-return-total phase23"><span>Tổng hàng trả</span><b id="mReturnTotal">'+money(totalReturnAmount)+'</b></div><div class="m-return-total phase23 due"><span>Phải thu sau trả</span><b id="mProductDueAfterReturn">'+money(Math.max(0,amount(order,"receivable")-totalReturnAmount))+'</b></div><div class="m-action-row workflow-next phase23"><button type="submit" class="m-confirm">Xác nhận hàng & thu tiền</button><button id="mFullReturnOrder" type="button" class="danger">Trả hết đơn</button></div></form>'
;var formEl=el("mProductReturnForm");formEl.addEventListener("submit",function(event){saveReturn(event,{nextTab:"payment",
successMessage:"Đã xác nhận hàng trả, chuyển sang Thu tiền"})}),bindReturnTotal(formEl,"mReturnTotal"),el("mFullReturnOrder").addEventListener("click",fullReturnOrder)
}else body.innerHTML='<div class="m-empty">Chọn khách/đơn ở danh sách cần giao trước.</div>'}(body):"returns"===state.tab?function(body){var order=currentOrder();if(order){
var rows=returnsForOrder(order);!rows.length&&Array.isArray(order.returnItems)&&order.returnItems.length&&(rows=order.returnItems.map(function(item){return Object.assign({},item,{
...
}),bindReturnTotal(formEl,"mReturnTotal"),el("mSkipReturns").addEventListener("click",function(){
