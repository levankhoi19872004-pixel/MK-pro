...
};bind(el("mReload"),"click",function(){load({force:!0,refreshActiveTab:!0})}),bind(el("mReconShortcut"),"click",function(){state.tab="reconciliation",render(),
loadDeliveryReconciliation(!1)}),bind(el("mLogout"),"click",logout),bind(el("mDate"),"change",function(){load({force:!0})}),bind(el("mStatusFilter"),"change",function(){load({
force:!0})});var debouncedSearch=mobileUiRuntime?mobileUiRuntime.debounce(function(){load({force:!0})},250):debounce(function(){load({force:!0})},250)
...
"reconciliation"===state.tab&&loadDeliveryReconciliation(!1)}),deliveryLifecycle.listen(window,"pagehide",function(){deliveryOrderRenderer&&deliveryOrderRenderer.cancel(),
...
el("mKpiTh")&&(el("mKpiTh").textContent=money(s.th)),el("mKpiCn")&&(el("mKpiCn").textContent=money(s.cn)),document.querySelectorAll("[data-m-tab]").forEach(function(button){
button.classList.toggle("active",button.getAttribute("data-m-tab")===state.tab)}),function(){var bar=el("mWorkflowBar");if(bar){var order=currentOrder()
;if(!order||"orders"===state.tab||"reconciliation"===state.tab)return bar.hidden=!0,void(bar.innerHTML="")
;var phone=deliveryMobileUi.orderPhone(order),phoneUrl=deliveryMobileUi.phoneHref(phone),title=order.customerName||order.customerCode||order.orderCode||"Đơn đang chọn"
;bar.hidden=!1,
bar.innerHTML="<div><b>"+esc(title)+"</b><span>"+esc(order.orderCode||"")+" · "+money(amount(order,"receivable"))+'</span></div><div class="m-workflow-actions">'+(phoneUrl?'<a href="'+esc(phoneUrl)+'">Gọi</a>':"")+'<button type="button" data-workflow-tab="products">Hàng</button><button type="button" data-workflow-tab="returns">Trả</button><button type="button" data-workflow-tab="payment" class="primary">Thu</button></div>'
...
var items=Array.isArray(order.items)?order.items:[],totalQty=items.reduce(function(sum,it){return sum+num(it.quantity||it.deliveredQty||it.qty||it.orderQty||it.soldQty)
},0),totalAmount=items.reduce(function(sum,it){
return sum+num(it.unitPrice||it.price||it.salePrice||it.finalPrice)*num(it.quantity||it.deliveredQty||it.qty||it.orderQty||it.soldQty)},0)
;body.innerHTML=selectedOrderSummary(order)+'<section class="m-workflow-step"><b>Bước 1/4 · Kiểm hàng giao</b><span>NVGH xem đủ hàng trước khi nhập trả hàng hoặc thu tiền.</span></section><section class="m-product-summary"><div><span>Số dòng</span><b>'+esc(items.length)+"</b></div><div><span>Tổng SL</span><b>"+money(totalQty)+"</b></div><div><span>Giá trị hàng</span><b>"+money(totalAmount)+'</b></div></section><div class="m-return-scroll products-readonly">'+(items.map(function(it){
var code=it.productCode||it.code||it.productId||"",name=it.productName||it.name||"",price=num(it.unitPrice||it.price||it.salePrice||it.finalPrice),qty=num(it.quantity||it.deliveredQty||it.qty||it.orderQty||it.soldQty)
...
}).join("")||'<div class="m-empty">Đơn chưa có dòng hàng để đối chiếu.</div>')+'</div><div class="m-action-row workflow-next"><button id="mGoReturnInput" type="button">Nhập hàng trả</button><button id="mGoPayment" type="button" class="secondary">Sang thu tiền</button></div>',
el("mGoReturnInput").addEventListener("click",function(){state.tab="returns",render(),loadSelectedReturnsDirect({force:!1})}),el("mGoPayment").addEventListener("click",function(){
state.tab="payment",render()})}else body.innerHTML='<div class="m-empty">Chọn đơn ở tab Đơn giao trước.</div>'
}(body):"returns"===state.tab?renderReturns(body):"payment"===state.tab?function(body){var order=currentOrder();if(order){
var receivable=amount(order,"receivable"),returnAmount=amount(order,"returnAmount")
;body.innerHTML=selectedOrderSummary(order)+'<section class="m-workflow-step"><b>Bước 3/4 · Thu tiền & xác nhận</b><span>App sẽ lưu tiền rồi xác nhận giao. Nếu thu thiếu, phần còn lại chuyển sang công nợ theo logic backend hiện có.</span></section><section class="m-product-summary payment-context"><div><span>Phải thu</span><b>'+money(receivable)+"</b></div><div><span>Hàng trả</span><b>"+money(returnAmount)+'</b></div><div><span>Còn phải xử lý</span><b id="mPaymentRemainingTop">0</b></div></section><form id="mPaymentForm" class="m-payment-form"><h3>Thu tiền đơn giao</h3><label>Tiền mặt<input name="cash" type="number" min="0" value="'+esc(amount(order,"cash"))+'"></label><label>Chuyển khoản<input name="bank" type="number" min="0" value="'+esc(amount(order,"bank"))+'"></label><label>Trả thưởng<input name="reward" type="number" min="0" value="'+esc(amount(order,"reward"))+'"></label><label>Còn thiếu / ghi công nợ<input id="mPaymentRemaining" type="text" readonly value="0"></label><button type="submit" class="m-confirm">Lưu thu tiền & xác nhận giao</button></form>'
...
}(body):"debt"===state.tab?function(body){var rows=state.debts||[],summary=state.debtSummary||{};if(!state.debtLoading||rows.length){
...
;return ids.some(function(id){return rowIds.indexOf(id)>=0})||codes.some(function(code){return rowCodes.indexOf(code)>=0})})}function renderReturns(body){var order=currentOrder()
;if(order){var rows=returnsForOrder(order);if(!rows.length&&Array.isArray(order.returnItems)&&order.returnItems.length&&(rows=order.returnItems.map(function(item){
...
})),!rows.length&&amount(order,"returnAmount")>0)return body.innerHTML=selectedOrderSummary(order)+'<div class="m-empty">Đơn có tiền hàng trả '+money(amount(order,"returnAmount"))+' nhưng app chưa lấy được dòng sản phẩm. Bấm Tải lại hàng trả để gọi trực tiếp returnOrders.</div><div class="m-action-row"><button id="mReloadReturns" type="button">Tải lại hàng trả</button><button id="mUseOrderItems" type="button" class="secondary">Nhập theo hàng giao</button></div>',
el("mReloadReturns").addEventListener("click",loadSelectedReturnsDirect),void el("mUseOrderItems").addEventListener("click",function(){
window.DeliveryCore.state.returns=window.DeliveryCore.state.returns||[],renderReturns(body)});rows=function(order,rows){
return Array.isArray(rows)&&rows.length||(rows=(Array.isArray(order&&order.items)?order.items:[]).map(function(item){return{
productCode:item.productCode||item.code||item.productId||"",productName:item.productName||item.name||"",price:num(item.unitPrice||item.price||item.salePrice||item.finalPrice),
returnQty:num(item.returnQty||item.qtyReturn||item.returnQuantity||item.returnedQty||0),deliveredQty:num(item.quantity||item.deliveredQty||item.qty||item.orderQty||item.soldQty)}
})),rows}(order,rows);var totalReturnAmount=rows.reduce(function(sum,it){return sum+num(it.returnQty)*num(it.price)},0)
;body.innerHTML=selectedOrderSummary(order)+'<section class="m-workflow-step"><b>Bước 2/4 · Hàng trả nếu có</b><span>Không có hàng trả thì bấm Bỏ qua để sang Thu tiền. Có trả thì nhập SL trả theo từng sản phẩm.</span></section><form id="mReturnSaveForm"><div class="m-return-scroll">'+(rows.map(function(it,idx){
var qtyText=null!=it.deliveredQty?" · SL giao "+money(it.deliveredQty):"",amount=num(it.returnQty)*num(it.price)
...
}).join("")||'<div class="m-empty">Đơn chưa có dòng hàng để nhập trả hàng.</div>')+'</div><div class="m-return-total"><span>Tổng hàng trả</span><b id="mReturnTotal">'+money(totalReturnAmount)+'</b></div><div class="m-action-row"><button type="submit">Lưu hàng trả</button><button id="mSkipReturns" type="button" class="secondary">Bỏ qua hàng trả</button></div></form>',
el("mReturnSaveForm").addEventListener("submit",saveReturn),el("mSkipReturns").addEventListener("click",function(){
...
}else body.innerHTML='<div class="m-empty">Chọn đơn ở tab Đơn giao trước.</div>'}async function saveReturn(event){event&&event.preventDefault&&event.preventDefault();try{
