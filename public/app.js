...
const salesPromotionBox=document.getElementById('salesPromotionBox');

const promotionForm=document.getElementById('promotionForm');
const promotionFormTitle=document.getElementById('promotionFormTitle');
...
const clearImportPreviewButton=document.getElementById('clearImportPreviewButton');
const selectedTemplateInfo=document.getElementById('selectedTemplateInfo');
const importTemplateGrid=document.getElementById('importTemplateGrid');
...
const IMPORT_TEMPLATE_UI={
  products:{title:'Mẫu import sản phẩm',desc:'Cột bắt buộc: mã sản phẩm, tên sản phẩm. Có thể nhập thêm đơn vị, barcode, nhóm hàng, giá nhập, giá bán.',icon:'📦'},
  customers:{title:'Mẫu import khách hàng',desc:'Cột bắt buộc: mã khách hàng, tên khách hàng. Nên có SĐT, địa chỉ, khu vực và nhân viên phụ trách.',icon:'🏪'},
  openingStock:{title:'Mẫu import tồn kho ban đầu',desc:'Dùng để khai báo tồn đầu kỳ. Mã sản phẩm phải tồn tại trước trong danh mục sản phẩm.',icon:'📊'},
  importOrders:{title:'Mẫu import phiếu nhập kho',desc:'Dùng cho nhập kho từ Excel. Các dòng cùng mã phiếu sẽ được gom thành một phiếu nhập.',icon:'📥'},
  salesOrders:{title:'Mẫu import đơn bán hàng',desc:'Dùng cho đơn bán/đơn con. Hệ thống kiểm tra tồn kho trước khi cho xác nhận import.',icon:'🧾'},
  openingDebt:{title:'Mẫu import công nợ ban đầu',desc:'Dùng để nhập số dư công nợ đầu kỳ theo từng khách hàng.',icon:'💳'},
  debtCollections:{title:'Mẫu import thu công nợ',desc:'Dùng cho tiền nhân viên giao hàng hoặc khách hàng nộp; đồng thời ghi công nợ và quỹ tiền.',icon:'💵'},
  cashbook:{title:'Mẫu import quỹ tiền',desc:'Dùng cho các khoản thu/chi quỹ tiền trong ngày, hỗ trợ thủ quỹ kiểm soát dòng tiền.',icon:'🏦'}
};
...
function setupTabs(){
  document.querySelectorAll('.tab-button').forEach(button=>{
    button.addEventListener('click',async()=>{
      ...
      if(button.dataset.tab==='salesTab'){await loadProducts();await loadCustomers();await loadPromotions();renderSalesProductSelect();renderSalesCustomerSelect()}
      if(button.dataset.tab==='promotionsTab') await loadPromotions()
    });
  });
}
...
function calculatePromotionPreview(){
  const todayValue=today();
  const applied=[];
  let discount=0;
  const gross=salesItems.reduce((s,i)=>s+Number(i.amount||0),0);
  (promotionsCache||[]).filter(p=>p.isActive!==false).forEach(p=>{
    if(p.startDate && todayValue<p.startDate)return;
    if(p.endDate && todayValue>p.endDate)return;
    const codes=new Set((p.productCodes||[]).map(code=>String(code||'').toLowerCase()));
    const matched=salesItems.filter(i=>codes.has(String(i.productCode||'').toLowerCase())||codes.has(String(i.productId||'').toLowerCase()));
    const qty=matched.reduce((sum,i)=>sum+Number(i.quantity||0),0);
    const base=matched.reduce((sum,i)=>sum+Number(i.amount||0),0);
    if(!matched.length || qty<Number(p.minQty||0) || base<=0)return;
    let amount=0;
    if(p.type==='discount_percent')amount=Math.round(base*Number(p.discountPercent||0)/100);
    if(p.type==='discount_amount')amount=Math.min(Number(p.discountAmount||0),base);
    if(amount>0){discount+=amount;applied.push({...p,previewDiscount:amount,previewQty:qty})}
  });
  discount=Math.min(discount,gross);
  return {gross,discount,total:Math.max(0,gross-discount),applied};
}
function updateSalesPromotionPreview(){
  if(!salesPromotionBox)return;
  if(!salesItems.length){salesPromotionBox.textContent='Chưa có khuyến mại áp dụng.';return}
  const result=calculatePromotionPreview();
  if(!result.applied.length){salesPromotionBox.textContent='Chưa có khuyến mại áp dụng.';return}
  salesPromotionBox.innerHTML=`<strong>Khuyến mại:</strong> ${result.applied.map(p=>`${p.code} - giảm ${money(p.previewDiscount)}`).join(' · ')}<br><span>Trước KM: ${money(result.gross)} · Giảm: ${money(result.discount)} · Sau KM: <strong>${money(result.total)}</strong></span>`;
}
...
function getPromotionPayload(){
  const payload=Object.fromEntries(new FormData(promotionForm).entries());
  payload.minQty=Number(payload.minQty||0);
  payload.discountPercent=Number(payload.discountPercent||0);
  payload.discountAmount=Number(payload.discountAmount||0);
  payload.isActive=promotionForm.elements.isActive.checked;
  return payload;
}
function resetPromotionForm(){
  ...
  promotionForm.elements.isActive.checked=true;
  if(promotionFormTitle)promotionFormTitle.textContent='Thêm chương trình khuyến mại';
  showMessage(promotionMessage,'');
}
function renderPromotions(){
  if(!promotionList)return;
  const q=String(promotionSearchInput?.value||'').trim().toLowerCase();
  const promotions=(promotionsCache||[]).filter(p=>{
    const text=[p.code,p.name,(p.productCodes||[]).join(' '),p.note].join(' ').toLowerCase();
    return !q || text.includes(q);
  });
  if(promotionCount)promotionCount.textContent=`${promotions.length} / ${(promotionsCache||[]).length} CTKM`;
  if(!promotions.length){promotionList.innerHTML='<div class="empty-state">Chưa có chương trình khuyến mại phù hợp.</div>';return}
  promotionList.innerHTML=promotions.map(p=>`
    <article class="promotion-card">
      <div class="promotion-head">
        <div><strong>${p.code}</strong><span>${p.name}</span></div>
        <span class="badge ${p.isActive!==false?'in':'out'}">${p.isActive!==false?'Đang áp dụng':'Đã tắt'}</span>
      </div>
      <div class="promotion-rule">
        <span>SP: ${(p.productCodes||[]).join(', ')}</span>
        <span>Điều kiện: từ ${money(p.minQty)} sản phẩm</span>
        <span>${p.type==='discount_percent'?`Giảm ${money(p.discountPercent)}%`:`Giảm ${money(p.discountAmount)}`}</span>
      </div>
      <div class="order-meta">${p.startDate||'Không giới hạn'} → ${p.endDate||'Không giới hạn'} ${p.note?` · ${p.note}`:''}</div>
      <div class="promotion-actions">
        <button class="small" onclick="editPromotion('${p.id}')">Sửa</button>
        <button class="small ${p.isActive!==false?'danger':'success'}" onclick="togglePromotion('${p.id}', ${p.isActive!==false?'false':'true'})">${p.isActive!==false?'Tắt':'Bật'}</button>
      </div>
    </article>`).join('');
}
async function loadPromotions(){
  try{
    const res=await fetch('/api/promotions');const json=await res.json();if(!json.ok)throw new Error(json.message||'Không tải được khuyến mại');
    promotionsCache=json.promotions||[];
    renderPromotions();
    updateSalesPromotionPreview();
  }catch(err){if(promotionCount)promotionCount.textContent='Lỗi tải CTKM';if(promotionList)promotionList.innerHTML=err.message}
}
async function submitPromotion(event){
  event.preventDefault();
  const payload=getPromotionPayload();
  const id=promotionForm.elements.id.value;
  const url=id?`/api/promotions/${encodeURIComponent(id)}`:'/api/promotions';
  const method=id?'PUT':'POST';
  try{
    const res=await fetch(url,{method,headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    const json=await res.json();if(!json.ok)throw new Error(json.message||'Không lưu được CTKM');
    showMessage(promotionMessage,json.message||'Đã lưu CTKM');
    resetPromotionForm();
    await loadPromotions();
  }catch(err){showMessage(promotionMessage,err.message,true)}
}
window.editPromotion=(id)=>{
  const p=(promotionsCache||[]).find(item=>item.id===id);
  if(!p || !promotionForm)return;
  promotionForm.elements.id.value=p.id;
  promotionForm.elements.code.value=p.code||'';
  promotionForm.elements.name.value=p.name||'';
  promotionForm.elements.type.value=p.type||'discount_percent';
  promotionForm.elements.productCodes.value=(p.productCodes||[]).join('\n');
  promotionForm.elements.minQty.value=p.minQty||1;
  promotionForm.elements.discountPercent.value=p.discountPercent||0;
  promotionForm.elements.discountAmount.value=p.discountAmount||0;
  promotionForm.elements.startDate.value=p.startDate||'';
  promotionForm.elements.endDate.value=p.endDate||'';
  promotionForm.elements.note.value=p.note||'';
  promotionForm.elements.isActive.checked=p.isActive!==false;
  if(promotionFormTitle)promotionFormTitle.textContent=`Sửa CTKM ${p.code}`;
  document.querySelector('[data-tab="promotionsTab"]')?.click();
};
window.togglePromotion=async(id,isActive)=>{
  try{
    const res=await fetch(`/api/promotions/${encodeURIComponent(id)}/status`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({isActive})});
    const json=await res.json();if(!json.ok)throw new Error(json.message||'Không đổi trạng thái CTKM');
    await loadPromotions();
      ...
      <article class="sales-order-card">
        ...
        <div class="sales-order-main">
          ...
          <div class="sales-order-money">
            <div><span>Tổng tiền</span><strong>${money(o.totalAmount)}</strong>${Number(o.discountAmount||0)>0?`<small>Giảm KM: ${money(o.discountAmount)}</small>`:''}</div>
...
function resetImportPreview(){
  ...
  if(importPreviewSummary)importPreviewSummary.innerHTML='<span>Tổng dòng: <strong>0</strong></span><span>Hợp lệ: <strong>0</strong></span><span>Lỗi: <strong>0</strong></span>';
  if(importPreviewHead)importPreviewHead.innerHTML='<tr><th>Trạng thái</th><th>Dữ liệu</th><th>Lỗi</th></tr>';
  if(importPreviewTable)importPreviewTable.innerHTML='<tr><td colspan="3">Chưa có dữ liệu preview.</td></tr>';
...
function updateSelectedTemplateUI(){
  ...
  const info=IMPORT_TEMPLATE_UI[type]||IMPORT_TEMPLATE_UI.products;
  if(selectedTemplateInfo){
    selectedTemplateInfo.innerHTML=`<div class="template-icon">${info.icon}</div><div><strong>${info.title}</strong><p>${info.desc}</p></div>`;
  }
  document.querySelectorAll('.template-download-card').forEach(btn=>btn.classList.toggle('active',btn.dataset.template===type));
}
function downloadTemplateByType(type){
  if(!type)return;
  window.location.href=`/api/import/template/${encodeURIComponent(type)}`;
}
...
function downloadImportTemplate(){
  if(!importDataType)return;
  downloadTemplateByType(importDataType.value);
}
...
async function commitImportExcel(){
  ...
  try{
    ...
    if(commitImportButton)commitImportButton.disabled=true;
    await loadProducts();await loadCustomers();await loadPromotions();await loadStock();await loadImportOrders();await loadSalesOrders();await loadDebts();await loadCashbook();
...
if(clearImportPreviewButton)clearImportPreviewButton.addEventListener('click',resetImportPreview);
if(importTemplateGrid)importTemplateGrid.addEventListener('click',event=>{
  const btn=event.target.closest('.template-download-card');
  if(!btn)return;
  if(importDataType){importDataType.value=btn.dataset.template;updateSelectedTemplateUI();}
  downloadTemplateByType(btn.dataset.template);
});
if(importDataType)importDataType.addEventListener('change',()=>{resetImportPreview();updateSelectedTemplateUI();});
...
if(promotionSearchInput)promotionSearchInput.addEventListener('input',renderPromotions);
